# Datamate Railway template seed
...
| `API`                 | `ghcr.io/datamate-analytics/datamate-api:latest` after CI publish, or repo + `api.Dockerfile` for seed testing | Yes, port `3001`      | RPC/API, auth callbacks, AI, admin APIs   |
| `Events`              | `ghcr.io/datamate-analytics/datamate-basket:latest`                                                            | Yes, port `4000`      | Analytics event ingestion/webhooks        |
| `Insights`            | `ghcr.io/datamate-analytics/datamate-insights:latest`                                                          | No, port `4002`       | Investigation scheduler and worker        |
...
| `Links`               | `ghcr.io/datamate-analytics/datamate-links:latest`                                                             | Optional, port `2500` | Short-link redirects/tracking             |

Use `Events` as the display name instead of `basket` in the template UI. Users understand what it does.
Keep `Links` optional unless the template supports Datamate Links out of the box.

## Icons and grouping

- Template icon: Datamate mark.
- `Dashboard`: Datamate/browser icon.
...
DATAMATE_ENCRYPTION_KEY=${{secret(64)}}
...
`bun run --filter @datamate/status build` and the start command to
`bun run --filter @datamate/status start`.
...
BETTER_AUTH_URL=https://${{Dashboard.RAILWAY_PUBLIC_DOMAIN}}
BETTER_AUTH_SECRET=${{shared.BETTER_AUTH_SECRET}}
DATAMATE_ENCRYPTION_KEY=${{shared.DATAMATE_ENCRYPTION_KEY}}
SELFHOST=${{shared.SELFHOST}}
...
EMAIL_FROM=Datamate <no-reply@example.com>
ALERTS_EMAIL_FROM=Datamate <alerts@example.com>
```
