		...
		if (isDashboardFeatureSource(path)) {
			if (
				...
				report(
					...
					"Use a component from @datamate/ui instead of a native interactive element."
				);
			}
			...
			if (ts.isImportDeclaration(node) && isDirectPrimitiveImport(node)) {
				report(
					node,
					RULE.noRawInteractiveHtml,
					"Import interactive primitives through @datamate/ui, not Radix or Base UI directly."
				);
			}
