...
The Datamate REST API provides programmatic access to analytics data.
...
1. Go to [Dashboard → Organization Settings → API Keys](https://app.datamate.cc/organizations/settings#api-keys)
...
```http
POST https://basket.datamate.cc/?client_id={website_id}
```
...
```http
POST https://basket.datamate.cc/batch?client_id={website_id}
```
