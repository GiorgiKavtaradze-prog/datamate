...
The Node.js SDK (`@datamate/sdk/node`) provides server-side tracking with batching, middleware, and deduplication support.
...
```typescript
import { Datamate } from "@datamate/sdk/node";
import type { Middleware } from "@datamate/sdk/node";
...
export async function handler(event) {
  const client = new Datamate({ clientId: "..." });
...
```typescript
import { db } from "@datamate/sdk/node";
