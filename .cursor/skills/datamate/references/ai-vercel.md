...
The AI SDK integration (`@datamate/sdk/ai/vercel`) provides LLM observability for applications using the Vercel AI SDK.

## Installation

```bash
bun add @datamate/sdk ai @ai-sdk/openai
```
...
const { track } = datamateLLM({ apiKey: "..." });
const model = track(openai("gpt-4o"));
...
```typescript
import type { Transport } from "@datamate/sdk/ai/vercel";
...
```typescript
import { httpTransport } from "@datamate/sdk/ai/vercel";
...
const { track } = datamateLLM({ transport });
```
...
```typescript
import { generateTraceId } from "@datamate/sdk/ai/vercel";
