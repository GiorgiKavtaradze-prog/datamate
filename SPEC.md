# Datamate Intelligence

## Product job

Datamate explains what changed and why it matters, then turns material problems into work that stays open until resolved.
...
The agent may inspect code without write credentials. For a code action it returns a patch and verification plan. Datamate validates and applies the patch, creates the branch and PR, records updates in the investigation, and resumes the agent on review feedback.
...
Reject output that merely restates a percentage, invents a cause, asks for data Datamate can read, gives a generic recommendation, or creates duplicate work.
