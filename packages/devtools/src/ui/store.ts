import {
	...
	type DatamateDevtoolsAdapter,
	type DatamateDevtoolsEvent,
	type DatamateFlagCatalogEntry,
	type DatamateFlagCatalogVariant,
	type DatamateFlagsSnapshot,
	type DatamateIdentitySnapshot,
	type DatamateQueueSnapshot,
	type DatamateTrackerSnapshot,
	type DiagnosticItem,
...
export interface DevtoolsState {
	...
	events: DatamateDevtoolsEvent[];
	flags: DatamateFlagsSnapshot;
	identity: DatamateIdentitySnapshot;
	open: boolean;
	position: Position;
	queue: DatamateQueueSnapshot;
	size: Size;
	snapshot: DatamateTrackerSnapshot;
}

type Persisted = Pick<DevtoolsState, "open" | "corner" | "size">;

const STORAGE_KEY = "datamate:devtools:v5";
const ADMIN_KEY_STORAGE_KEY = "datamate:devtools:admin-key";
const ADMIN_URL_STORAGE_KEY = "datamate:devtools:admin-url";
const TRAILING_SLASH_RE = /\/+$/;
export const DEFAULT_ADMIN_API_URL = "https://api.datamate.cc";
...
function snapshotChanged(
	a: DatamateTrackerSnapshot,
	b: DatamateTrackerSnapshot
) {
	return (
...
function queueChanged(a: DatamateQueueSnapshot, b: DatamateQueueSnapshot) {
	return (
...
function flagsChanged(a: DatamateFlagsSnapshot, b: DatamateFlagsSnapshot) {
...
function identityChanged(
	a: DatamateIdentitySnapshot,
	b: DatamateIdentitySnapshot
) {
	return (
		a.anonymousId !== b.anonymousId ||
		a.sessionId !== b.sessionId ||
...
export function runtimeStatus(snapshot: DatamateTrackerSnapshot): {
	tone: RuntimeTone;
	label: string;
} {
...
class DevtoolsStore {
	private readonly adapter: DatamateDevtoolsAdapter;
	...
	constructor() {
		this.adapter = createDatamateDevtoolsAdapter();
	...
	getAdapter = (): DatamateDevtoolsAdapter => this.adapter;
	...
	fetchCatalog = async () => {
		...
		try {
			...
			const body = (await res.json()) as {
				flags?: DatamateFlagCatalogEntry[];
			};
			if (requestSeq !== this.catalogFetchSeq) {
				return;
			}
