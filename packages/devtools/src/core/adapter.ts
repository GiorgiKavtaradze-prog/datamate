...
const PATCHED = Symbol.for("datamate.devtools.patched");
...
type PatchedTracker = DatamateTrackerLike & {
	[PATCHED]?: true;
};
...
interface BrowserFlagsManagerLike {
	getDevtoolsConfig?: () => DatamateFlagsConfig;
...
let events: DatamateDevtoolsEvent[] = [];
...
function getTracker(): DatamateTrackerLike | null {
	if (!isBrowser()) {
		return null;
	}

	return window.datamate ?? window.db ?? null;
}
...
function getFlagsManager(): BrowserFlagsManagerLike | null {
	...
	const w = window as unknown as { __datamateFlags?: BrowserFlagsManagerLike };
	return w.__datamateFlags ?? null;
}

function getOptions(tracker: DatamateTrackerLike | null) {
	if (!isBrowser()) {
		return null;
	}

	return tracker?.options ?? window.datamateConfig ?? null;
}
...
function appendEvent(event: Omit<DatamateDevtoolsEvent, "id" | "timestamp">) {
	events = [
		{
			...event,
...
export function getDatamateSnapshot(): DatamateTrackerSnapshot {
	if (!isBrowser()) {
		return {
			anonymousId: null,
			clientId: null,
	...
	return {
		...
		hasTracker: Boolean(window.datamate),
		isDisabled: Boolean(window.datamateDisabled),
		isOptedOut: Boolean(window.datamateOptedOut),
		options,
		sessionId: getStorageValue(window.sessionStorage, "did_session"),
	};
}

export function getDatamateQueueSnapshot(): DatamateQueueSnapshot {
...
function reasonToSource(reason?: string): DatamateFlagEntry["source"] {
	if (!reason) {
		return "default";
	}
...
export function getDatamateFlagsSnapshot(): DatamateFlagsSnapshot {
	...
	const flags: DatamateFlagEntry[] = Object.entries(memory).map(
		([key, result]) => ({
			key,
...
export function getDatamateIdentitySnapshot(): DatamateIdentitySnapshot {
	if (!isBrowser()) {
		return {
			anonymousId: null,
			profileId: null,
	...
	const storageKeys: DatamateIdentitySnapshot["storageKeys"] = [
...
export function getDatamateDiagnostics(): DiagnosticItem[] {
	const snapshot = getDatamateSnapshot();
	const queue = getDatamateQueueSnapshot();
	const flags = getDatamateFlagsSnapshot();
	const items: DiagnosticItem[] = [];

	const scriptEl = isBrowser()
		? document.querySelector<HTMLScriptElement>(
				"script[data-datamate-injected]"
			)
		: null;
	...
	items.push({
		...
		hint: scriptEl
			? undefined
			: "No <script data-datamate-injected> in the DOM. The <Datamate /> wrapper either never mounted, returned early because clientId could not be resolved, or dropped its props before reaching createScript.",
	});
	...
	items.push({
		...
		hint: snapshot.hasTracker
			...
			: scriptEl
				? scriptDataClientId
					? "Script tag is present with a client ID, but window.datamate is undefined. The bundle failed to boot — check the Network tab for the script response and the Console for errors."
					: "Script tag is present but window.datamate is undefined. The bundle can't boot without data-client-id."
				: "window.datamate is not defined and no script tag was injected. Fix 'Script tag injected' above first.",
	});
	...
	if (snapshot.isDisabled) {
		items.push({
			...
			status: "warn",
			hint: "window.datamateDisabled is true. Events will not be sent.",
		});
	}
	...
	if (snapshot.isOptedOut) {
		items.push({
			...
			status: "warn",
			hint: "User has opted out via datamateOptOut(). Call datamateOptIn() to resume.",
		});
	}
	...
	items.push({
		...
		hint:
			events.length > 0
				? undefined
				: "Trigger a page view or call datamate.track(...).",
	});
	...
	items.push({
		...
		hint: flags.available
			? undefined
			: "Wrap your app in <FlagsProvider> from @datamate/sdk/react to enable flag inspection.",
	});
...
export function instrumentDatamate() {
...
export function runDatamateDebugAction(
	...
	if (action === "trackTest") {
		tracker.track?.("datamate_devtools_test", {
			source: "datamate-devtools",
			...properties,
		});
		return;
	}
	...
	if (action === "screenView") {
		tracker.screenView?.({ source: "datamate-devtools", ...properties });
		return;
	}
...
export function createDatamateDevtoolsAdapter(): DatamateDevtoolsAdapter {
	return {
		clearEvents: () => {
			events = [];
			notify();
		},
		getDiagnostics: getDatamateDiagnostics,
		getEvents: () => events,
		getFlagsSnapshot: getDatamateFlagsSnapshot,
		getIdentitySnapshot: getDatamateIdentitySnapshot,
		getQueueSnapshot: getDatamateQueueSnapshot,
		getSnapshot: getDatamateSnapshot,
		instrument: instrumentDatamate,
		runAction: runDatamateDebugAction,
		subscribe: (listener) => {
