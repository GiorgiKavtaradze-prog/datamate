...
const TEST_PAGE_HTML = `<!DOCTYPE html>
                ...
                <head>
                    ...
                    <title>Datamate Test Page</title>
                </head>
                <body>
                    <h1>Test Page</h1>
...
const TRACKER_TEST_HTML = `
        ...
        <html lang="en">
          <head>
            <meta charset="UTF-8">
            <meta name="viewport" content="width=device-width, initial-scale=1.0">
            <title>Datamate Tracker Test Suite</title>
          ...
          <body class="max-w-5xl mx-auto p-8 antialiased">
            <div class="flex justify-between items-center mb-8">
                <h1 class="text-3xl font-bold flex items-center gap-3">
                    <span>🔍</span> Datamate Tracker Test
                </h1>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                <div class="bg-card p-6 rounded-xl border border-slate-700 shadow-lg hover:border-blue-500/50  ">
                    ...
                    <div class="space-y-3">
                        <button class="w-full bg-blue-600 hover:bg-blue-700 text-white px-4 py-2.5 rounded-lg transition-all font-medium text-left flex justify-between group" onclick="datamate.track('test_click', { button: 'primary' })">
                            Track Custom Event
                            <span class="opacity-0 group-hover:opacity-100 transition-opacity">→</span>
                        </button>
                        <button class="w-full bg-slate-700 hover:bg-slate-600 text-white px-4 py-2.5 rounded-lg transition-all font-medium text-left" onclick="datamate.track('purchase', { amount: 99.99, currency: 'USD' })">Track Purchase</button>
                        <button class="w-full bg-slate-700 hover:bg-slate-600 text-white px-4 py-2.5 rounded-lg transition-all font-medium text-left" onclick="datamate.track('signup', { method: 'email' })">Track Signup</button>
                ...
                <div class="bg-card p-6 rounded-xl border border-slate-700 shadow-lg hover:border-red-500/50  ">
                    ...
                    <div class="space-y-3">
                        ...
                        <button class="w-full bg-slate-700 hover:bg-slate-600 text-white px-4 py-2.5 rounded-lg transition-all font-medium text-left" onclick="datamate.track('error_log', { error: 'manual' })">Track Manual Error</button>
                    </div>
                </div>
            ...
            <script>
                ...
                function updateStatus() {
                    const isOptedOut = localStorage.getItem("datamate_opt_out") === "true";
                ...
                function toggleOptOut() {
                    if (localStorage.getItem("datamate_opt_out") === "true") {
                        datamateOptIn();
                    } else {
                        datamateOptOut();
                    }
                ...
                function checkState() {
                    console.log("Current Config:", datamate?.options);
                ...
                console.log = (...args) => {
                    ...
                    if (args[0]?.toString().includes('[Datamate]')) {
                        appendLog(args);
                    }
                };
