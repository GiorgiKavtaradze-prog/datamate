...
test.describe("API Methods", () => {
	...
	test.describe("flush sends all queued events", () => {
		test("flush() delivers queued track() calls via /track", async ({
			...
			await page.route("**/basket.datamate.cc/track**", async (route) => {
		...
		test("flush() delivers queued vitals via /vitals", async ({
			...
			await page.route("**/basket.datamate.cc/vitals**", async (route) => {
				vitalsFired = true;
				await route.fulfill({
					status: 200,
					body: JSON.stringify({ success: true }),
		...
		test("flush() delivers queued errors via /errors", async ({
			...
			await page.route("**/basket.datamate.cc/errors**", async (route) => {
				errorsFired = true;
				await route.fulfill({
					status: 200,
					body: JSON.stringify({ success: true }),
	...
	test.describe("destroy", () => {
		test("removes global window.datamate reference", async ({ page }) => {
			await page.goto("/test");
			await page.evaluate(() => {
				(window as any).datamateConfig = {
			...
			const exists = await page.evaluate(() => {
				const tracker = (window as any).datamate;
				// Access the actual Datamate instance if possible
				// For now, we test that window.datamate is cleared
