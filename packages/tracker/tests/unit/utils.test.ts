...
import { Datamate } from "../../src/index";
import {
	buildPagePath,
	getTrackerConfig,
	isOptedOut,
...
function constructOptedOutTracker({
	...
	const local = createStorage({
		...STORED_IDENTITY,
		...(explicit ? { datamate_opt_out: "true" } : {}),
	});
	...
	Object.defineProperty(globalThis, "window", {
		configurable: true,
		value: { datamateOptedOut: explicit },
	});
	Object.defineProperty(globalThis, "navigator", {
		configurable: true,
	...
	const tracker = new Datamate({ clientId: "site_example" });
...
describe("getTrackerConfig", () => {
	test("preserves empty strings in URL query params while data attributes treat them as true", () => {
		...
		setTrackerScript({
			...
			src: "https://cdn.example.com/datamate-debug.js?clientId=&apiUrl=&ignoreBotDetection=&batchSize=10&disabled=false",
		});
