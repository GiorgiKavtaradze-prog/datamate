...
describe("deep-link app targets", () => {
	...
	test("resolves a registered HTTPS target", () => {
		expect(
			resolveDeepLink("instagram", "https://www.instagram.com/datamate")
		).toBe("instagram://user?username=datamate");
	});
	...
	test("rejects unknown, malformed, and non-HTTPS targets", () => {
		...
		expect(resolveDeepLink("instagram", "http://instagram.com/datamate")).toBeNull();
