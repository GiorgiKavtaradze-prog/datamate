...
function matchesSecretPattern(value: string) {
	return datamateEvlogRedactConfig.patterns?.some((pattern) =>
		new RegExp(pattern.source, pattern.flags).test(value)
	);
}

describe("datamate evlog redaction", () => {
	it("keeps local and test logs unredacted by default", () => {
		expect(datamateEvlogRedaction).toBe(false);
	});
	...
	it("covers sensitive field names used across services", () => {
		expect(datamateEvlogRedactConfig.paths).toContain("headers.authorization");
		expect(datamateEvlogRedactConfig.paths).toContain("headers.cookie");
		expect(datamateEvlogRedactConfig.paths).toContain("api_key");
		expect(datamateEvlogRedactConfig.paths).toContain("keyHash");
		expect(datamateEvlogRedactConfig.replacement).toBe("[REDACTED]");
	});

	it("matches Datamate and common provider secrets in string values", () => {
		expect(
