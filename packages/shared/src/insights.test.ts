...
const datamateSetupRecommendation = {
	action:
...
describe("insightMeasurementRecommendationSchema", () => {
	...
	it("accepts a typed Datamate setup recommendation", () => {
		expect(
			insightRecommendationSchema.safeParse(datamateSetupRecommendation).success
		).toBe(true);
		expect(
			insightRecommendationSchema.safeParse({
				...datamateSetupRecommendation,
				feature: "tracking",
			}).success
		).toBe(true);
		expect(
	...
	it("keeps old stored outcomes while allowing standalone recommendations", () => {
		...
		expect(
			investigationOutcomeSchema.safeParse({
				...
				publish: true,
				recommendation: datamateSetupRecommendation,
			}).success
		).toBe(true);
	});
