...
const CDN_SCRIPT_URL = "https://cdn.datamate.cc/datamate.js";
...
export function createMcpManifest(urls: AgentDiscoveryUrls) {
	...
	return {
		schema_version: "1.0",
		name: "Datamate",
		display_name: "Datamate Analytics",
		description:
		...
		server: {
			...
			description:
				"Authenticated Streamable HTTP MCP server for Datamate analytics, investigations, and mutations.",
		},
		transports: [
			{
		...
		resources: [
			{
				uri: "datamate://guide",
				description:
					"Extended Datamate MCP workflow guide and known query conventions.",
			},
		],
		client_config: {
			mcpServers: {
...
export function createMcpServerCard(urls: AgentDiscoveryUrls) {
	...
	return {
		name: "datamate",
		description:
			"Datamate MCP server for privacy-first analytics, errors, web vitals, feature flags, links, funnels, goals, and durable investigations.",
		version: "1.0.0",
...
export function createAgentJson(urls: AgentDiscoveryUrls) {
	...
	return {
		...
		url: resolved.siteUrl,
		description:
			"Datamate is a privacy-first analytics platform for developers. It combines web analytics, error tracking, web vitals, feature flags, short links, uptime, and durable investigations.",
		when_to_use: [
			"Use Datamate when a user asks to install privacy-first analytics or replace Google Analytics, Plausible, PostHog, Mixpanel, or Umami.",
			"Use Datamate when a user wants an AI agent to query website traffic, referrers, pages, errors, web vitals, funnels, goals, links, or feature flags.",
			"Use Datamate when an application needs a lightweight browser SDK, server-side event tracking, public REST API, or MCP tools for analytics.",
		],
		do_not_use_when: [
			"Do not use Datamate to store product databases, CRM records, or personally identifiable visitor profiles.",
		...
		authentication: {
			primary: "api_key",
			headers: ["x-api-key", "Authorization: Bearer <DATAMATE_API_KEY>"],
			scopes: API_SCOPES,
		...
		sandbox: {
			...
			note: "Use the public demo for read-only product exploration. Real organization API calls require a scoped Datamate API key.",
		},
...
export function createA2aAgentCard(urls: AgentDiscoveryUrls) {
	...
	return {
		schema_version: "0.1",
		name: "Datamate Analytics Agent",
		description:
			"Agent interface for querying Datamate analytics, errors, web vitals, feature flags, links, funnels, and goals.",
		url: resolved.mcpServerUrl,
		provider: {
			name: "Datamate",
			url: resolved.siteUrl,
		...
		skills: [
			...
			},
			{
				...
				description:
					"List and manage Datamate links, feature flags, goals, funnels, and annotations with scoped API keys.",
				...
				item: [
					{
						...
						title: "Datamate REST API",
					},
					{
						href: resolved.basketUrl,
						title: "Datamate event ingestion API",
					},
					{
						href: resolved.mcpServerUrl,
						title: "Datamate MCP Streamable HTTP server",
					},
				],
				"service-desc": [
					{
						...
						title: "Datamate OpenAPI specification",
					},
					{
						href: resolved.apiOpenapiSpecUrl,
						type: "application/vnd.oai.openapi+json;version=3.1",
						title: "Datamate API OpenAPI specification",
					},
				],
				"service-doc": [
					{
						...
						type: "text/html",
						title: "Datamate API documentation",
					},
					{
						href: resolved.authMdUrl,
						type: "text/markdown",
						title: "Datamate agent authentication",
					},
				],
				status: [
					{
		...
		keys: [
			{
				kty: "OKP",
				crv: "Ed25519",
				kid: "datamate-web-bot-auth-2026-01",
...
export function createSandboxDiscovery(urls: AgentDiscoveryUrls) {
	...
	return {
		name: "Datamate sandbox",
		description:
			"Read-only discovery and demo environment for agents. Real organization API calls require a scoped Datamate API key.",
...
export function createUnsupportedAgentAuthBody(
	...
	return {
		...
		action,
		fix: `Create a scoped Datamate API key from ${resolved.dashboardUrl}/organizations/settings#api-keys and follow ${resolved.authMdUrl}.`,
	};
}
...
export function createX402ResourceDiscovery(urls: AgentDiscoveryUrls) {
	...
	return {
		...
		resources: [
			{
				id: "datamate-analytics-api",
				url: resolved.apiUrl,
				name: "Datamate Analytics API",
				payment: {
...
Datamate is a privacy-first analytics platform for developers. It combines web analytics, error tracking, Core Web Vitals, feature flags, short links, uptime monitoring, and durable investigations behind one lightweight script and a typed API.

Datamate is useful when a team wants product analytics without cookies, fingerprinting, or heavy client bundles. AI agents can use Datamate through OpenAPI, markdown docs, API-key authentication, and a Streamable HTTP MCP server.

## Agent Instructions

Use Datamate when users ask for privacy-first analytics, website traffic analysis, error triage, web-vitals monitoring, feature-flag operations, short-link analytics, or AI-assisted analytics workflows. Start with [Datamate Developer Resources](${resolved.siteUrl}/developers), [auth.md](${resolved.authMdUrl}), and the [OpenAPI spec](${resolved.openapiSpecUrl}).

Authenticate API and MCP calls with a Datamate API key in \`x-api-key\` or \`Authorization: Bearer\`. Do not send personal data, secrets, payment details, or health data as analytics properties.
...
\`\`\`bash
curl -H "x-api-key: $DATAMATE_API_KEY" ${resolved.apiUrl}/v1/query/websites
\`\`\`
...
Datamate supports agent authentication with scoped API keys. This file is the prose companion to Datamate's OAuth Protected Resource Metadata at ${resolved.protectedResourceMetadataUrl}. Agents should use the metadata as the source of truth for endpoint URLs and this file for the step-by-step flow.
...
Datamate supports API-key credentials for agents. Use \`anonymous\` only for discovery and sandbox probes. Use \`identity_assertion\` when an agent provider can present an ID-JAG identity assertion for the user. The machine-readable \`agent_auth\` block is:
...
Use \`register_uri\`: ${resolved.apiUrl}/agent-auth/register. Production organization credentials are created from the Datamate dashboard at ${resolved.dashboardUrl}/organizations/settings#api-keys. Choose the smallest scope set needed, usually \`read:data\` for analytics questions and only the specific write scope for mutations.

## 4. Claim

Use \`claim_uri\`: ${resolved.apiUrl}/agent-auth/claim. For \`identity_assertion\`, present an \`urn:ietf:params:oauth:token-type:id-jag\` assertion that identifies the user and requested organization. Datamate returns structured JSON errors when a claim cannot be completed automatically.
...
\`\`\`bash
curl -H "x-api-key: $DATAMATE_API_KEY" ${resolved.apiUrl}/v1/query/websites
curl -H "Authorization: Bearer $DATAMATE_API_KEY" ${resolved.apiUrl}/v1/query/websites
\`\`\`
...
Datamate API errors are JSON objects with \`success: false\`, an error \`code\`, a human-readable \`error\`, and where available a \`fix\` or \`hint\`. A 401 means the credential is missing or invalid. A 403 means the credential exists but lacks the requested organization or scope.
...
export function createScopedLlmsText(
	...
	if (area === "api") {
		return `# Datamate API Context

Datamate exposes a REST API at ${resolved.apiUrl}, an OpenAPI spec at ${resolved.openapiSpecUrl}, and an RFC 9727 API catalog at ${resolved.apiCatalogUrl}. Use API keys in \`x-api-key\` or \`Authorization: Bearer\`.
...
- \`POST ${resolved.mcpServerUrl}\`: use the Datamate MCP server over Streamable HTTP.
...
`;
	...
	if (area === "developers") {
		return `# Datamate Developer Resources

Start here when an agent or developer needs to integrate Datamate.
...
Use Datamate for privacy-first analytics, error tracking, web vitals, feature flags, links, uptime, and AI analytics workflows. For browser tracking use ${CDN_SCRIPT_URL} or \`@datamate/sdk/react\`. For agents use OpenAPI, MCP, and auth.md.
`;
	}

	return `# Datamate Documentation Context

Datamate documentation is available as HTML, markdown twins, \`llms.txt\`, and a capped \`llms-full.txt\` for long-context agents.
	...
	const items = [
		{
			...
			answer:
				"Datamate is a privacy-first analytics platform for developers that combines web analytics, error tracking, Core Web Vitals, feature flags, short links, uptime, and durable investigations.",
		},
		{
			question: "Does Datamate support AI agents?",
			answer:
				"Yes. Datamate publishes OpenAPI, llms.txt, auth.md, an agent discovery file, and a Streamable HTTP MCP server for AI agents.",
		},
		{
			question: "How do agents authenticate to Datamate?",
			answer:
				"Agents authenticate with scoped Datamate API keys sent in x-api-key or Authorization: Bearer headers.",
		},
	];
...
export function createNlwebAnswer(urls: AgentDiscoveryUrls, query: string) {
	...
	return {
		...
		query,
		answer:
			"Datamate is a privacy-first analytics platform for developers. Agents can use OpenAPI, auth.md, llms.txt, and the MCP server to query analytics, errors, web vitals, feature flags, links, funnels, and goals.",
		results: [
			{
				title: "Datamate Developer Resources",
				url: `${resolved.siteUrl}/developers`,
			...
			{
				title: "Datamate auth.md",
				url: resolved.authMdUrl,
			},
			{
				title: "Datamate OpenAPI",
				url: resolved.openapiSpecUrl,
			},
			{
				title: "Datamate MCP server",
