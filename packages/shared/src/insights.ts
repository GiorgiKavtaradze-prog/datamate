...
export const insightDatamateSetupRecommendationSchema = z
	.object({
		action: measurementRecommendationActionSchema.describe(
			"Exact evidence-backed Datamate setup and the future customer question it unlocks."
		),
		feature: z.enum(["tracking", "user_identification"]),
		kind: z.literal("datamate_setup"),
	})
	.strict();
...
const investigationActNextSchema = z.object({
	...
	execution: insightDefinitionExecutionSchema
		...
		.optional()
		.describe(
			"Exact edit or delete Datamate can apply when this action is clicked. Use only for the signal's existing goal or funnel; null is accepted only for agent transport and normalized before persistence."
		),
});
...
const agentInvestigationNextSchema = z.discriminatedUnion("type", [
	investigationActNextSchema.extend({
		...
		execution: agentInsightDefinitionExecutionSchema
			.nullable()
			.describe(
				"Exact edit or delete Datamate can apply when this action is clicked, or null when no definition edit applies."
			),
	}),
	investigationAskNextSchema,
	investigationResolveNextSchema,
...
export const insightRecommendationSchema = z
	.union([
		...
		insightDatamateSetupRecommendationSchema,
	])
	.nullable()
	.describe(
		"Concrete evidence-backed next step worth suggesting without opening an investigation. Datamate setup must use a backend-verified setup or instrumentation candidate that names the exact blind spot and future decision it unlocks. Use null when there is no useful next step."
	);
	...
	.superRefine((outcome, context) => {
		...
		if (
			...
			(!("kind" in outcome.recommendation) ||
				outcome.recommendation.kind !== "datamate_setup")
		) {
			context.addIssue({
				code: "custom",
				message:
					"Actions and questions can only carry a backend-verified Datamate setup recommendation",
				path: ["recommendation"],
			});
		}
...
>;
export type InsightDatamateSetupRecommendation = z.infer<
	typeof insightDatamateSetupRecommendationSchema
>;
