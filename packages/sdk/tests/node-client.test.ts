...
import { Datamate } from "../src/node/index";
import type {
	BatchEventInput,
	BatchEventResponse,
} from "../src/node/types";
...
describe("Datamate Node client", () => {
	it("rejects blank API keys after trimming", () => {
		expect(() => new Datamate({ apiKey: "   " })).toThrow("apiKey");
	});
	...
	it("returns a failed flush result when track reaches the batch threshold", async () => {
		...
		const client = new Datamate({ apiKey: "dbdy_test", batchSize: 1 });
	...
	it("starts a new flush for an event queued as the active flush settles", async () => {
		...
		let batchCalls = 0;
		let lateDelivery: ReturnType<Datamate["track"]> | undefined;
	...
	it("preserves generated identity when the same public batch is retried", async () => {
		...
		const client = new Datamate({ apiKey: "dbdy_test" });
	...
	it("reports queued events separately from delivered events", async () => {
		...
		const client = new Datamate({ apiKey: "dbdy_test", batchSize: 10 });
	...
	it("does not throw when debug logging receives unserializable data", () => {
		...
		const client = new Datamate({ apiKey: "dbdy_test", debug: true });
...
describe("identify", () => {
	it("sends profileId, anonymousId, traits, and websiteId to /identify", async () => {
		...
		const client = new Datamate({ apiKey: "dbdy_test", websiteId: "site_1" });
