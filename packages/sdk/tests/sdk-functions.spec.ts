...
test.describe("SDK Functions", () => {
	...
	test.describe("isTrackerAvailable", () => {
		...
		test("returns true when window.datamate exists", async ({ page }) => {
			const available = await page.evaluate(() => {
				(window as any).datamate = {
					track: () => {},
					screenView: () => {},
					setGlobalProperties: () => {},
					clear: () => {},
	...
	test.describe("track", () => {
		test("calls window.datamate.track with name and properties", async ({
			page,
		}) => {
	...
	test.describe("clear", () => {
		test("calls window.datamate.clear", async ({ page }) => {
			const result = await page.evaluate(() => {
				let called = false;
				(window as any).datamate = {
					clear: () => {
	...
	test.describe("flush", () => {
		test("calls window.datamate.flush", async ({ page }) => {
			const result = await page.evaluate(() => {
				let called = false;
				(window as any).datamate = {
					flush: () => {
	...
	test.describe("createScript", () => {
		test("creates script element with correct src", async ({ page }) => {
			...
			expect(result.src).toContain("cdn.datamate.cc/datamate.js");
