export interface DatamateConfig {
	/** Whether Datamate anonymizes visitor IDs before storage. Default: true. */
	anonymizeVisitorIds?: boolean | "auto";
	/** Event ingestion endpoint. Default: `'https://basket.datamate.cc'` */
	apiUrl?: string;
	...
	batchTimeout?: number;
	/** Datamate Client ID. Auto-detected from `NEXT_PUBLIC_DATAMATE_CLIENT_ID` env var if omitted. */
	clientId?: string;
	...
	samplingRate?: number;
	/** Custom browser bundle URL. Default: `'https://cdn.datamate.cc/datamate.js'` */
	scriptUrl?: string;
...
/** The global tracker instance at `window.datamate` or `window.db`. */
/**
...
export interface DatamateTracker {
...
declare global {
	interface Window {
		datamate?: DatamateTracker;
		db?: {
			track: DatamateTracker["track"];
			screenView: DatamateTracker["screenView"];
			identify: DatamateTracker["identify"];
			setTraits: DatamateTracker["setTraits"];
			clearProfile: DatamateTracker["clearProfile"];
			getProfileId: DatamateTracker["getProfileId"];
			clear: DatamateTracker["clear"];
			flush: DatamateTracker["flush"];
			setGlobalProperties: DatamateTracker["setGlobalProperties"];
		};
	}
}
