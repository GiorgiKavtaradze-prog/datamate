...
import type { DatamateTracker, ProfileTraits } from "./types";

/** Check if the full tracker instance (`window.datamate`) is available. */
export function isTrackerAvailable(): boolean {
	return typeof window !== "undefined" && !!(window.datamate || window.db);
}

/** Returns the `window.datamate` tracker instance, or `null` if unavailable. */
export function getTracker(): DatamateTracker | null {
	if (typeof window === "undefined") {
		return null;
	}
	return window.datamate || null;
}
...
function callTracker(
	...
	const tracker = window.db ?? window.datamate;
	if (!tracker) {
...
export function getProfileId(): string | null {
	...
	const fn = window.db?.getProfileId || window.datamate?.getProfileId;
	if (fn) {
		try {
			return fn();
		} catch (error) {
