...
export const Datamate = defineComponent({
	props: {
		apiUrl: String,
		batchSize: Number,
		batchTimeout: Number,
		...
		const injectScript = () => {
			...
			if (!clientId) {
				if (!props.disabled && props.debug) {
					console.warn(
						"Datamate: No client ID found. Please provide clientId prop or set NEXT_PUBLIC_DATAMATE_CLIENT_ID / NUXT_PUBLIC_DATAMATE_CLIENT_ID / VITE_DATAMATE_CLIENT_ID environment variable."
					);
				}
				return;
			}
