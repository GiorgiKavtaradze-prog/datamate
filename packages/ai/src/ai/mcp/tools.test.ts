...
import { z } from "zod";
import { handleDatamateMcpRequest } from "../../mcp/http";
	...
	test("create_link matches the HTTP(S) and deep-link app contract", () => {
		...
		expect(
			createLink.inputSchema.safeParse({
				...
				targetUrl: "https://instagram.com/datamate",
				websiteId: "website-1",
			}).success
		).toBe(true);
	});
...
describe("investigation tools", () => {
	test("publishes the investigation lifecycle to a website-scoped key", async () => {
		...
		const response = await handleDatamateMcpRequest({
			apiKey: principal.apiKey,
			organizationId: "org-1",
			request: new Request("https://api.datamate.test/v1/mcp", {
				body: JSON.stringify({
					id: 1,
					jsonrpc: "2.0",
					method: "tools/list",
