...
import type { ApiKeyRow } from "@datamate/api-keys/resolve";
import type { LanguageModelUsage, StepResult, ToolSet } from "ai";
import { ToolLoopAgent } from "ai";
import { DatamateAgentUserError } from "../../agent/errors";
...
import type { DatamateAgentSlackContext } from "./slack-context";
...
async function prepareMcpAgentRun(options: RunMcpAgentOptions) {
	...
	if (
		...
		throw new DatamateAgentUserError({
	...
	const agent = new ToolLoopAgent({
		...
		experimental_telemetry: {
			isEnabled: true,
			functionId: `datamate.${source}.ask`,
...
const FEEDBACK_REQUEST_PATTERN =
	/\b(feedback|feature request|report (this|that|it|a bug)|file a bug|send (this|that|it) to|tell the (datamate )?team|complain)\b/i;
