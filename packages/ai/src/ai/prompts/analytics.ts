...
const FEEDBACK_TOOL_RULES = `**Feedback to the Datamate team (submit_feedback):**
- When the user says part of Datamate looks broken, asks for a capability that does not exist, or keeps hitting an error that blocks them, offer once to send their report to the Datamate team.
...
- Example/preview asks ("what would an investigation look like", "show me an example") => explain that Datamate does not fabricate previews and offer a real one-off investigation. Call configure_investigations action=run only when the user explicitly asks; start with confirmed=false.
...
<website_domain>Obtain from list_websites result</website_domain>`;
	...
	return `You are Databunny, an analytics assistant for Datamate.
