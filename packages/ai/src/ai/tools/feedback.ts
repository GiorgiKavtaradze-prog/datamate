...
import { submitFeedback } from "@datamate/services/feedback";
import { tool } from "ai";
import { z } from "zod";
	...
	const submitFeedbackTool = tool({
		...
		execute: async ({ errorDetails, websiteId, ...input }, options) => {
			...
			try {
				...
				return {
					success: true,
					message: "Feedback sent to the Datamate team.",
					feedbackId: row.id,
				};
			} catch (error) {
