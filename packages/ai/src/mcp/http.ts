...
const DEFAULT_MCP_SERVER_NAME = "datamate";
const DEFAULT_MCP_SERVER_VERSION = "1.0.0";

export interface DatamateMcpHttpOptions extends McpRequestContext {
	request: Request;
	serverName?: string;
...
export async function createMcpUnauthorizedResponse(
	...
	return Response.json(
		...
		},
		{
			status: 401,
			headers: {
				"WWW-Authenticate": `Bearer realm="datamate", error="invalid_token", error_description="API key required (x-api-key or Authorization: Bearer)"${resourceMetadata}`,
			},
		}
	);
}
...
export async function handleDatamateMcpRequest(
	options: DatamateMcpHttpOptions
): Promise<Response> {
	mergeWideEvent({
