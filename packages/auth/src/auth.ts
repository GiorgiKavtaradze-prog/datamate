...
import { and, db, eq, like } from "@datamate/db";
// biome-ignore lint/performance/noNamespaceImport: drizzleAdapter requires the full schema map object.
import * as schema from "@datamate/db/schema";
import {
...
import { SlackProvider } from "@datamate/notifications";
import {
	getRedisCache,
...
} from "@datamate/services/audit";
import {
	auditActions,
	type AuditActionDefinition,
	type AuditActor,
} from "@datamate/shared/audit";
...
export const auth = betterAuth({
	...
	advanced: {
		crossSubDomainCookies: {
			...
			domain: process.env.BETTER_AUTH_COOKIE_DOMAIN ?? ".datamate.cc",
		},
		cookiePrefix: isProduction() ? "datamate" : "datamate-dev",
	...
	plugins: [
		...
		emailOTP({
			...
			async sendVerificationOTP({ email, otp, type }) {
				...
				if (type === "sign-in") {
					subject = `${otp} — Sign in to Datamate`;
		...
		magicLink({
			...
			sendMagicLink: async ({ email, url }) => {
				...
				await sendAuthEmail({
					to: email,
					subject: "Your sign-in link for Datamate",
