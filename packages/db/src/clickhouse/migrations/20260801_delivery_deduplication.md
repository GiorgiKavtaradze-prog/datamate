...
5. Confirm `analytics` uses the `Atomic` or `Replicated` database engine and
   ...
   `datamate_cluster`; stop if the live name differs.
...
```sql
DROP TABLE analytics.daily_pageviews_mv ON CLUSTER datamate_cluster SYNC;
DROP TABLE analytics.events_delivery_mirror_mv ON CLUSTER datamate_cluster SYNC;
DROP TABLE analytics.link_visits_delivery_mirror_mv ON CLUSTER datamate_cluster SYNC;
DROP TABLE analytics.outgoing_links_delivery_mirror_mv ON CLUSTER datamate_cluster SYNC;
DROP TABLE analytics.custom_events_delivery_mirror_mv ON CLUSTER datamate_cluster SYNC;
DROP TABLE analytics.error_spans_delivery_mirror_mv ON CLUSTER datamate_cluster SYNC;
DROP TABLE analytics.web_vitals_spans_delivery_mirror_mv ON CLUSTER datamate_cluster SYNC;
DROP TABLE analytics.daily_pageviews_delivery_mirror_mv ON CLUSTER datamate_cluster SYNC;
```
...
```sql
EXCHANGE TABLES analytics.events AND analytics.events_delivery_v2 ON CLUSTER datamate_cluster;
EXCHANGE TABLES analytics.link_visits AND analytics.link_visits_delivery_v2 ON CLUSTER datamate_cluster;
EXCHANGE TABLES analytics.outgoing_links AND analytics.outgoing_links_delivery_v2 ON CLUSTER datamate_cluster;
EXCHANGE TABLES analytics.custom_events AND analytics.custom_events_delivery_v2 ON CLUSTER datamate_cluster;
EXCHANGE TABLES analytics.error_spans AND analytics.error_spans_delivery_v2 ON CLUSTER datamate_cluster;
EXCHANGE TABLES analytics.web_vitals_spans AND analytics.web_vitals_spans_delivery_v2 ON CLUSTER datamate_cluster;
EXCHANGE TABLES analytics.daily_pageviews AND analytics.daily_pageviews_delivery_v2 ON CLUSTER datamate_cluster;
```
