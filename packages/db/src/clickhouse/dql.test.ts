...
describe("DQL ClickHouse client", () => {
	test("requires a direct dedicated-user connection", async () => {
		for (const url of [
			undefined,
			"not-a-url",
			"http://default:admin@localhost:8123/datamate_analytics",
			"http://dql_user:@localhost:8123/datamate_analytics",
			"http://dql_user:password@localhost:8123/datamate_analytics?user=default",
			"http://dql_user:password@clickhouse:8123/datamate_analytics",
		]) {
		...
		const clients = [
			createDqlClient(
				"http://dql_user:password@localhost:8123/datamate_analytics"
			),
			createDqlClient(
				"https://dql_user:password@clickhouse.example.com:8443/datamate_analytics"
			),
	...
	test("rejects writes and unpublished ClickHouse surfaces before execution", async () => {
		...
		const rejected = [
			...
			"SELECT path FROM analytics.events SETTINGS SQL_datamate_website_id = 'other'",
...
const LOCAL_CLICKHOUSE_URL =
	...
	"http://default:@localhost:8123/datamate_analytics";
