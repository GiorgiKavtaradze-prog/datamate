...
`@datamate/cache` extends Drizzle ORM's base Cache class to provide Redis-backed caching for database queries. It automatically caches query results and invalidates cache entries when mutations occur on tracked tables, ensuring data consistency while improving query performance.
...
```bash
bun add @datamate/cache drizzle-orm
```
...
```typescript
import type { RedisCacheConfig } from "@datamate/cache";
