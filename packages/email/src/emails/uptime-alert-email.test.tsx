...
const SAFE_HREF_FALLBACK = "https://app.datamate.cc/";
...
describe("UptimeAlertEmail — field fallbacks and optional props", () => {
	...
	test("dashboardUrl absent → no dashboard button", async () => {
		...
		expect(html).not.toContain("View in Datamate");
	});
	...
	test("dashboardUrl present → dashboard button appears with that href", async () => {
		const html = await render({
			kind: "down",
			url: "https://example.com",
			dashboardUrl: "https://app.datamate.cc/monitors/abc123",
		});
		expect(html).toContain("View in Datamate");
		expect(html).toContain("https://app.datamate.cc/monitors/abc123");
	});
});
