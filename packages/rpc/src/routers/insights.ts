...
function definitionChangeReply(type: RecheckableDefinitionType): string {
	return `Datamate detected a ${type} definition change. Recheck the current evidence and resolve this investigation if the change addressed it.`;
}
...
export async function appendInvestigationReply(
	...
	const stored = await db.transaction(async (tx) => {
		...
		if (active) {
			throw rpcError.badRequest(
				"Datamate is already investigating the latest reply"
			);
		}
...
export async function applyInsightAction(input: {
	...
	const completed = await db.transaction(async (tx) => {
		...
		if (activeReply) {
			throw rpcError.conflict(
				"Datamate is already verifying this investigation"
			);
		}
		...
		const body = `Datamate applied the ${entityType} action. Recheck its verification condition against current data.`;
		await tx.insert(insightReplies).values({
			...author,
			body,
		...
		.handler(async ({ context, input }) => {
			...
			const recommendationState = (source: typeof pageSource) => {
				...
				const isFreshStandaloneRecommendation = or(
					sql`coalesce(${recommendationKind}, '') not in ('instrumentation', 'datamate_setup')`,
				...
				const hasNativeRecommendation = or(
					...
					sql`${recommendationKind} in ('goal_draft', 'funnel_draft', 'instrumentation', 'datamate_setup')`
				);
