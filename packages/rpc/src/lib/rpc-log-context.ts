import type {
	RpcProcedureType,
	RpcWideEventFields,
} from "@datamate/shared/evlog-fields";
...
export function enrichRpcWideEventContext(
	...
	const clientId = context.headers.get("datamate-client-id");
	if (clientId) {
		fields.rpc_client_id = clientId;
	}

	const sdkName = context.headers.get("datamate-sdk-name");
	if (sdkName) {
		fields.rpc_sdk_name = sdkName;
	}

	const sdkVersion = context.headers.get("datamate-sdk-version");
	if (sdkVersion) {
