// Playground configuration for testing @datamate/nuxt locally.
//
// Architecture:
//   1. The inline <head> script below runs BEFORE any async external scripts,
//      so `Object.defineProperty` traps on `window.db` / `window.datamate` are
...
const DEBUG_SETUP_SCRIPT = `(function () {
  ...
  // (window.db and window.datamate may both fire for the same call)
  ...
  Object.defineProperty(window, "datamate", {
    configurable: true,
    get: function () { return _datamate; },
    // Only wrap if this is a different object from window.db
    set: function (t) { _datamate = t; if (t && t !== _db) wrap(t); },
  });
})();`;
