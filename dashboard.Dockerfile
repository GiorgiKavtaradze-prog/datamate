...
RUN bunx turbo prune @datamate/dashboard --docker
...
ARG NEXT_PUBLIC_API_URL=https://api.datamate.cc
ARG NEXT_PUBLIC_APP_URL=https://app.datamate.cc
ARG NEXT_PUBLIC_BASKET_URL=https://basket.datamate.cc
ARG NEXT_PUBLIC_LINKS_URL=https://dby.sh
ARG NEXT_PUBLIC_STATUS_URL=https://status.datamate.cc
...
RUN DATABASE_URL=postgres://datamate:datamate@localhost:5432/datamate \
	...
	bunx turbo run build --filter=@datamate/dashboard...
