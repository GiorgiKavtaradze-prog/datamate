...
import type { InvestigationSignal } from "@datamate/shared/insights";
import {
	summarizeAgentUsage,
	type UsageTelemetry,
} from "@datamate/ai/lib/usage-telemetry";
import type {
...
async function assertReadOnlyClickHouse(): Promise<void> {
	const { chQuery } = await import("@datamate/db/clickhouse");
...
async function inReadOnlyTransaction<T>(
	...
	const pool = new Pool({
		application_name: "datamate_insights_shadow_readonly",
...
async function closeShadowConnections(): Promise<void> {
	const [{ clickHouse }, { shutdownRedis }] = await Promise.all([
		import("@datamate/db/clickhouse"),
		import("@datamate/redis/redis"),
	]);
...
async function runProductionShadow(options: CliOptions): Promise<ShadowReport> {
	...
	try {
		...
		const { setClickHouseReadMode } = await import("@datamate/db/clickhouse");
