...
describe("intelligence agent", () => {
	...
	it("routes an uncovered conversion event to the Goal specialist", async () => {
		...
		const rawErrorOutcome = {
			...
			next: {
				question:
					"Can you connect the repository that owns this route so Datamate can inspect the failure path?",
				type: "ask" as const,
			},
			rootCause: null,
		};
		...
		it("requires matched continuation before a performance vital claims downstream impact", async () => {
			const measuredOutcome = {
				...
				next: {
					question:
						"Can you connect the repository that owns the sign-in route so Datamate can inspect the slow path?",
					type: "ask" as const,
				},
	...
	it("accepts only the supplied Datamate setup recommendation", async () => {
		const setupRecommendationCandidate = {
			action:
				"Verify or add Datamate identify() after authentication so future errors can be tied to signed-in users.",
			feature: "user_identification" as const,
			kind: "datamate_setup" as const,
		};
		const setupOutcome = {
			...agentOutcome,
			next: {
				question:
					"Connect the repository that owns the application so Datamate can inspect the failure path.",
				type: "ask" as const,
			},
		...
		await expect(
			...
			"Datamate setup recommendations must match the evidence-backed candidate exactly"
		);
	});

	it("keeps standalone Datamate setup recommendations out of the insight feed", async () => {
		const setupRecommendationCandidate = {
			action:
				"Verify or add Datamate identify() after authentication so future errors can be tied to signed-in users.",
			feature: "user_identification" as const,
		...
		await expect(
			...
			"standalone Datamate setup recommendations must stay unpublished"
		);
