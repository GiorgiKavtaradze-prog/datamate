import type { AppContext } from "@datamate/ai/config/context";
import {
	AI_MODEL_MAX_RETRIES,
	createModelFromId,
	isAiGatewayConfigured,
} from "@datamate/ai/config/models";
import { getAILogger } from "@datamate/ai/lib/ai-logger";
import {
	agentInvestigationOutcomeSchema,
	investigationOutcomeSchema,
	type AgentInvestigationOutcome,
	type InsightDatamateSetupRecommendation,
	type InsightDefinitionOperation,
	type InvestigationOutcome,
	type InvestigationSignal,
	type InsightMeasurementRecommendation,
} from "@datamate/shared/insights";
import {
	type LanguageModel,
...
export interface InsightAgentInput {
	...
	setupRecommendationCandidate?: InsightDatamateSetupRecommendation | null;
	signal: InvestigationSignal;
}
...
const INSTRUCTIONS = `Investigate one exact Datamate signal until a teammate has a clear next move or a useful new fact.
...
A recommendation is one concrete, non-interrupting improvement. It appears in Recommendations whether or not the investigation is published. Standalone setup and measurement recommendations resolve with publish false. A supplied setupRecommendationCandidate is backend-verified: copy that candidate exactly as kind datamate_setup. It may accompany the primary act or ask, but never replaces the repair. Do not infer identity, payment, or revenue setup from an error cohort. Require inspected source/configuration for code, hosting, browser, or integration recommendations. An existing goal/funnel that is materially unsafe for its explicitly established purpose is an executable exact edit/delete, not a recommendation. Delete only when inspection establishes it has no independent valid use; if its measured journey is useful but mislabeled, preserve it and edit the name/description to state the actual proxy. Cosmetic renames are not actions.
...
function datamateSetupRecommendation(
	recommendation: AgentInvestigationOutcome["recommendation"]
): InsightDatamateSetupRecommendation | null {
	return recommendation &&
		"kind" in recommendation &&
		recommendation.kind === "datamate_setup"
		? recommendation
		: null;
}
...
function validateMeasurementRecommendation(
	...
	const setupRecommendation = datamateSetupRecommendation(
	...
	if (setupRecommendation) {
		...
		if (
			...
			throw new Error(
				"Insights Datamate setup recommendations must match the evidence-backed candidate exactly"
			);
		}
		if (outcome.next.type === "resolve" && outcome.publish !== false) {
			throw new Error(
				"Insights standalone Datamate setup recommendations must stay unpublished"
			);
		}
	}
	if (
...
export async function runInsightAgent(
	...
	const availableTools =
		options.tools ??
		(await import("@datamate/ai/tools/toolkit")).createToolkit({
	...
	const createAgent = (finalOnly: boolean) =>
		new ToolLoopAgent({
			...
			experimental_telemetry: {
				...
				functionId: "datamate.insights.investigate",
			},
		});
	const prompt = {
