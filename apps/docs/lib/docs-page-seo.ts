...
export function getDocsPageSeo(page: DocPage): DocsPageSeoModel {
	...
	const url = `https://www.datamate.cc${page.url}`;
	const title = `${pageTitle} - Docs`;
	const description =
		page.data.description ??
		`${pageTitle} - guides and reference for Datamate, the privacy-first analytics platform.`;
	const ogImage = `https://www.datamate.cc${getPageImage(page).url}`;
