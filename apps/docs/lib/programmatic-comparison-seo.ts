...
export function getProgrammaticComparisonSeo(
	...
	if (variant === "alternative") {
		...
		return {
			title: `Alternative to ${name} (${YEAR}) [Pricing & Features] | Datamate`,
			description,
		};
	}

	const description = trimMetaDescription(
		`Switch from ${name} to Datamate (${YEAR}). Compare features, pricing, automatic investigations, and migration steps before you move.`
	);
	return {
		title: `Switch from ${name} to Datamate (${YEAR}) - Migration & Comparison`,
		description,
	};
}
...
export function getProgrammaticIntroText(
	...
	if (variant === "alternative") {
		return `Searching for an alternative to ${name}? Below is the same side-by-side breakdown teams use to compare privacy-first analytics, pricing, and features before they choose Datamate.`;
	}
