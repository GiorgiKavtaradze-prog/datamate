...
export async function generateMetadata({
	...
	if (!data) {
		return { title: "Not Found | Datamate" };
	}
...
export default async function SwitchFromPage({ params }: PageProps) {
	...
	return (
		<ComparisonPageView
			...
			faqs={faqs}
			featureSectionSubtitle={`Leaving ${competitor.name}? Feature and pricing comparison before you switch to Datamate`}
			features={features}
			...
			heroHeading={
				<>
					...
					<span className="text-muted-foreground">to</span> Datamate
				</>
			}
			introText={introText}
