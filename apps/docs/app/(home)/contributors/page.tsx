...
export const metadata: Metadata = {
	title: "Contributors | Datamate",
	description:
		"Meet the amazing developers building the future of privacy-first analytics",
	alternates: {
		canonical: "https://www.datamate.cc/contributors",
	},
	openGraph: {
		title: "Contributors | Datamate",
		description:
			"Meet the amazing developers building the future of privacy-first analytics",
		url: "https://www.datamate.cc/contributors",
		images: ["/og-image.png"],
	},
};
...
function fetchContributorsData(
	...
	const cachedContributorsResponse = cache(async () => {
		const contributorsResponse = await fetch(
			"https://api.github.com/repos/datamate-analytics/Datamate/contributors?per_page=100",
			requestInit
		);
...
function fetchLanguagesData(
	...
	const cachedLanguagesResponse = cache(async () => {
		const languagesResponse = await fetch(
			"https://api.github.com/repos/datamate-analytics/Datamate/languages",
			requestInit
		);
...
function fetchPullRequestsData(
	...
	const cachedPrsResponse = cache(async () => {
		const prsResponse = await fetch(
			"https://api.github.com/repos/datamate-analytics/Datamate/pulls?state=all&per_page=50",
			requestInit
		);
		if (!prsResponse.ok) {
...
function fetchCommitActivity(
	...
	const cachedCommitActivityResponse = cache(async () => {
		const response = await fetchWithRetry(
			"https://api.github.com/repos/datamate-analytics/Datamate/stats/commit_activity",
			statsRequestInit
		);
...
function fetchCodeFrequency(
	...
	const cachedCodeFrequencyResponse = cache(async () => {
		const response = await fetchWithRetry(
			"https://api.github.com/repos/datamate-analytics/Datamate/stats/code_frequency",
			statsRequestInit
		);
...
function fetchPunchCard(
	...
	const cachedPunchCardResponse = cache(async () => {
		const response = await fetchWithRetry(
			"https://api.github.com/repos/datamate-analytics/Datamate/stats/punch_card",
			statsRequestInit
		);
...
function fetchReleases(requestInit: RequestInit): Promise<ProcessedRelease[]> {
	const cachedReleasesResponse = cache(async () => {
		const response = await fetch(
			"https://api.github.com/repos/datamate-analytics/Datamate/releases?per_page=20",
			requestInit
		);
...
export default async function ContributorsPage() {
	...
	const title = "Contributors | Datamate";
	const description =
		"Meet the amazing developers building the future of privacy-first analytics";
	const url = "https://www.datamate.cc/contributors";
