...
function getDatamateMonthlyCost(monthlyEvents: number): {
	plan: NormalizedPlan;
	totalCost: number;
} {
...
export function calculateCookieBannerCost(
	...
	const { plan, totalCost } = getDatamateMonthlyCost(inputs.monthlyVisitors);

	return {
		...primary,
		lostRevenueYearlyRangeLow: lowBand.lostRevenueYearly,
		lostRevenueYearlyRangeHigh: highBand.lostRevenueYearly,
		datamateMonthlyCost: totalCost,
		datamatePlanName: plan.name,
	};
}
