...
import { ArrowRightIcon, WandSparkleIcon } from "@datamate/ui/icons";
import { Button, Card, Input } from "@datamate/ui";
...
export const LeapComponent = () => {
	const handleGenerate = () => {
		...
		const prompt = input?.value
			? `${input.value} use Datamate for analytics`
			: "";
		const url = new URL("https://leap.new/");
		url.searchParams.set("build", prompt);
		url.searchParams.set("utm_source", "datamate");
			...
			<Card.Content className="p-4">
				<div className="mb-2 flex items-center gap-2.5">
					...
					<h3 className="font-medium text-foreground text-sm">
						Try Datamate with Leap
					</h3>
				</div>

				<p className="mb-3 text-muted-foreground text-xs leading-5">
					Let Leap generate a complete application that uses Datamate for
					analytics.
				</p>
				...
				<div className="flex w-full flex-col gap-2 sm:flex-row">
					<Input
						...
						placeholder="What do you want to build with Datamate analytics?"
						type="text"
					/>
