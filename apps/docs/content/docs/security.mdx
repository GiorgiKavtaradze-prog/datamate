...
description: How Datamate protects your data and respects user privacy
---

import { CodeBlock } from "@/components/docs";

Datamate is built with privacy-first principles and enterprise-grade security. This guide covers what we do to protect your data and how to configure privacy settings for your users.
...
Datamate is **compliant by default** - no consent banners or cookie notices required because:
...
<CodeBlock 
  language="tsx"
  code={`import { Datamate } from "@datamate/sdk/react";
...
By default, Datamate only accepts requests from your website's registered domain. To track analytics from third-party services (like [Cal.com](/docs/Integrations/cal), embedded widgets, or other integrations), you need to add those domains to your allowed origins.

**Access**: Open [Websites](https://app.datamate.cc/websites), select your site, then go to **Settings → Security**.
...
Since Datamate only collects anonymous data, users don't need to request data deletion - there's no personal data to delete! However, you can still provide opt-out controls if desired.
...
<CodeBlock 
  ...
  code={`function PrivacyControls() {
    ...
    const handleOptOut = () => {
      ...
      localStorage.removeItem("datamate_anon_id");
...
- Anonymous analytics are collected via Datamate
...
- **Not required for Datamate** (no cookies used)
- Only needed if you use other tracking tools

### Sample Privacy Policy Text

```
Analytics: We use Datamate to collect anonymous website usage statistics
...
visitors. Data is processed securely by Datamate and used only for
...
<CodeBlock 
  ...
  code={`<!-- Add Datamate to your CSP -->
<meta
  http-equiv="Content-Security-Policy"
  content="script-src 'self' https://app.datamate.cc; 
    connect-src 'self' https://basket.datamate.cc;"
/>`}
/>
...
<CodeBlock 
  ...
  const config = {
    development: {
      clientId: process.env.NEXT_PUBLIC_DATAMATE_DEV_ID,
      disabled: true, // No tracking in development
    },
    production: {
      clientId: process.env.NEXT_PUBLIC_DATAMATE_PROD_ID,
      disabled: false,
    },
...
- 📧 [privacy@datamate.cc](mailto:privacy@datamate.cc)
...
**No data requests needed** - Datamate doesn't collect personal data that can identify users. If you have questions about our data handling:
...
- **[Getting Started](/docs/getting-started)** - Set up Datamate on your site
