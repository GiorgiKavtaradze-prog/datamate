...
<CodeBlock 
  language="javascript"
  code={`// Datamate automatically tracks LCP
...
<CodeBlock 
  language="javascript"
  code={`// Datamate tracks INP when Web Vitals are enabled
  datamate.track('core_web_vital', {
    metric: 'INP',
    value: inpValue,
    eventType: eventType
...
**Using Datamate:**
```javascript
// Datamate tracks CLS automatically
datamate.track('core_web_vital', {
  metric: 'CLS',
  value: clsValue,
...
datamate.track('seo_performance', {
  lcp: lcpValue,
  inp: inpValue,
  cls: clsValue,
...
```javascript
// Datamate provides automatic RUM
...
### Datamate's Performance Advantage
...
datamate.track('performance_improvement', {
  beforeLCP: 3.2,
  afterLCP: 2.1,
  beforeINP: 320,
  afterINP: 180,
...
Ready to optimize your Core Web Vitals? Datamate provides built-in performance monitoring without impacting your scores.

[Start monitoring your Core Web Vitals →](https://app.datamate.cc/register)
