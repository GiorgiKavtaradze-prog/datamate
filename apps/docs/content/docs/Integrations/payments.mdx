---
title: Payments
description: Attribute Stripe and Paddle revenue to sessions, campaigns, and identified users with Datamate webhooks.
---
...
> **TL;DR:** Generate a webhook URL in the dashboard, add it to Stripe or Paddle, and pass Datamate IDs in your payment metadata.

---

## 1. Generate your webhook URL

1. Open your [Datamate dashboard](https://app.datamate.cc) and go to **Website → Revenue**.
...
Datamate expects Stripe API version `2025-05-28.basil` or later. Keep `payment_intent.succeeded` enabled for one-time payments; `invoice_payment.paid` is the canonical event for payments attached to an invoice. Datamate uses the immutable InvoicePayment ID, so the same allocation embedded in `invoice.paid` and delivered later as `invoice_payment.paid` is counted once. `invoice.paid` also supplies the invoice-level remainder for payments made outside Stripe; Datamate records that remainder only when Stripe includes a complete payments list.

Paste the endpoint's **signing secret** into the Revenue settings so Datamate can verify each delivery.
...
Paste the **webhook secret key** into the Revenue settings so Datamate can verify each delivery.

</Tab>
</Tabs>

## 3. Pass Datamate IDs with each payment
...
const datamateMetadata = {
  datamate_client_id: websiteId,
  datamate_session_id: sessionId,
  datamate_anonymous_id: anonId,
...
- **Revenue appears but isn't attributed:** the payment had no Datamate metadata. Attribution only works for payments created after you started passing IDs.
