...
Add Datamate's privacy-first analytics to your [Mintlify](https://mintlify.com) documentation site by dropping a small `datamate.js` file next to your `docs.json`. Mintlify auto-loads any `.js` file in your content directory on every page, so the tracker becomes available across all of your docs with zero per-page wiring.

<Callout type="info">
  There is no official Datamate integration for Mintlify *yet*.
</Callout>

## How to Add Datamate to Mintlify

<Steps>
  <Step title="Get Your Client ID">
    Open your [Datamate dashboard](https://app.datamate.cc/websites), select the website that represents your docs, and copy the **Client ID** from the tracking setup screen.
  </Step>

  <Step title="Create datamate.js">
    In your Mintlify project, create a new file named `datamate.js` in the same directory as your `docs.json`.

    <CodeBlock
      language="javascript"
      filename="datamate.js"
      code={`(() => {
  ...
  <Step title="Deploy Your Docs">
    ...
    <CodeBlock
      language="bash"
      code={`git add datamate.js
git commit -m "feat(docs): add datamate analytics"
git push`}
    />
  ...
  <Step title="Verify Tracking">
    ...
    2. Reload the page and confirm a request to `cdn.datamate.cc/datamate.js` returns `200`.
    3. Visit your [Datamate dashboard](https://app.datamate.cc) and watch real-time visitors appear.
  </Step>
</Steps>
...
Once the script has loaded, `window.datamate` is available globally. You can call it from any inline `<script>` block in an MDX page or from your own custom script.

<CodeBlock
  language="javascript"
  code={`if (window.datamate) {
  window.datamate.track("docs_search", {
...
1. Confirm `datamate.js` is at the project root, next to `docs.json`.
...
2. Verify the domain on your Datamate site matches the deployed Mintlify domain.
...
Need help with your Mintlify integration? Contact us at [support@datamate.cc](mailto:support@datamate.cc).
