...
Add Datamate's privacy-first analytics to your Hugo static site by adding the tracking script to your site's templates. The best way to do this is by creating a partial template for the script and including it in your site's footer.

## How to Add Datamate to Hugo
...
></script>`}
  ...
  <Step title="Create a Partial Template">
    ...
    Inside `layouts/partials/`, create a new file named `datamate-analytics.html`.

    Paste your Datamate tracking script into this new file:

    <CodeBlock
      language="html"
      filename="layouts/partials/datamate-analytics.html"
      code={`{{/* layouts/partials/datamate-analytics.html */}}
...
{{ end }}`}
  ...
  <Step title="Include the Partial in Your Footer">
    ...
    Open your main base template or footer partial file. Just before the closing `</body>` tag, add the following line to include your Datamate Analytics partial:
...
<body>
    ...
    {{ partial "datamate-analytics.html" . }}
</body>
</html>`}
    />
...
<Steps>
  <Step title="Update datamate-analytics.html">
    Modify `layouts/partials/datamate-analytics.html` to use a site parameter:

    <CodeBlock
      language="html"
      filename="layouts/partials/datamate-analytics.html"
      code={`{{/* layouts/partials/datamate-analytics.html */}}
{{ if and (not .Site.IsServer) .Site.Params.datamateClientID }}
    <script
        src="https://cdn.datamate.cc/datamate.js"
        data-client-id="{{ .Site.Params.datamateClientID }}"
        async
    ></script>
{{ end }}`}
    />
  ...
  <Step title="Add Parameters to config.toml">
    ...
    <CodeBlock
      ...
      code={`[params]
  datamateClientID = "YOUR_CLIENT_ID"`}
    />
...
{{< datamate-track event="blog_post_view" >}}`}
/>
...
<CodeBlock
  language="html"
  filename="layouts/shortcodes/datamate-track.html"
  code={`{{/* layouts/shortcodes/datamate-track.html */}}
{{ if and (not .Site.IsServer) .Site.Params.datamateClientID }}
<script>
if (window.datamate) {
    window.datamate.track('{{ .Get "event" }}', {
...
Need help with your Hugo integration? Contact us at [support@datamate.cc](mailto:support@datamate.cc).
