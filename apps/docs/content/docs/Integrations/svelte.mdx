...
This guide explains how to integrate Datamate with your standard client-side Svelte applications, typically built using Vite (e.g., via `npm create vite@latest -- --template svelte`).

## How to Add Datamate to Your Client-Side Svelte App

The most straightforward method is to add the Datamate tracking script directly to your `index.html` file.
...
></script>`}
  ...
  <Step title="Add the Snippet to index.html">
    Open `index.html` and paste the Datamate tracking snippet just before the closing `</head>` tag or `</body>` tag.
...
<Callout type="info">
  **SPA Page View Tracking**: Datamate automatically tracks route changes in client-side Svelte SPAs. If you're using a custom router or page views aren't tracked correctly after navigation, you can manually trigger `window.datamate.screenView()` after each route change using your router's navigation events.
</Callout>
...
Always check for `window.datamate` before calling its methods.
...
<CodeBlock
  ...
  function toggleAccordion() {
    ...
    if (window.datamate) {
      window.datamate.track('accordion_toggle', {
        component: 'FAQ',
        expanded: expanded
      });
    }
...
<CodeBlock
  ...
  onMount(() => {
    // Inject Datamate script
    const script = document.createElement('script');
    script.src = 'https://cdn.datamate.cc/datamate.js';
...
- Datamate automatically tracks route changes for most routers
- If using a custom router, manually call `window.datamate.screenView()` on route changes
...
Need help with your Svelte integration? Contact us at [support@datamate.cc](mailto:support@datamate.cc).
