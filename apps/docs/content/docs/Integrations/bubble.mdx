...
This guide explains how to integrate Datamate with your Bubble.io application to track user behavior, page views, and custom events.

## How to Add Datamate to Your Bubble App
...
></script>`}
  ...
  <Step title="Add the Snippet to the Page Header">
    ...
    - Paste your Datamate tracking snippet into this box. This will add the script to the `<head>` section of every page in your Bubble app
  ...
  <Step title="Verify Installation">
    Open your live Bubble application and navigate through a few pages. Then, check your [Datamate dashboard](https://app.datamate.cc) to see if data is being received. You can also inspect your browser's network tab to confirm the script is loaded.
  </Step>
</Steps>

<Callout type="info">
  **Page View Tracking in Bubble Apps**: Bubble applications often behave like Single Page Applications (SPAs) for navigation between "pages" (which are often groups shown/hidden on a single actual HTML page). Datamate automatically detects URL changes in SPAs.

  - If your Bubble app uses actual URL route changes for different views, Datamate should track these automatically
  - If navigation primarily involves showing/hiding groups without URL changes, Datamate will only track the initial page load. For more granular tracking of "virtual" page views in such cases, you would need to use custom event tracking
  - If URL changes occur but are not automatically tracked, you can trigger `window.datamate.screenView()` manually using a Bubble workflow action that runs JavaScript when a "page" (group) becomes visible
</Callout>
...
<Steps>
  ...
  <Step title="Add Datamate Tracking Code">
    In the "Run JavaScript" action's code block, add your Datamate event tracking code:

    <CodeBlock
      language="javascript"
      code={`if (typeof window !== 'undefined' && window.datamate) {
  window.datamate.track('bubble_event_name', {
...
if (typeof window !== 'undefined' && window.datamate) {
  window.datamate.track('item_interaction', {
  ...
  <Step title="Test Your Event">
    ...
    - Check your [Datamate dashboard](https://app.datamate.cc) to see if the custom event data is being received
  </Step>
</Steps>
...
if (typeof window !== 'undefined' && window.datamate) {
  window.datamate.track('button_click', {
...
if (typeof window !== 'undefined' && window.datamate) {
  window.datamate.track('product_view', {
...
- Use `window.datamate.screenView()` manually when groups become visible
...
- Verify data types match what Datamate expects (strings, numbers, etc.)
...
Need help with your Bubble.io integration? Contact us at [support@datamate.cc](mailto:support@datamate.cc).
