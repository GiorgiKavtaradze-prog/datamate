...
Add Datamate's privacy-first analytics to your React application with TypeScript support and modern best practices.

## Installation

Install the Datamate SDK:
...
Add the `<Datamate />` component to inject the tracking script:
...
function App() {
  return (
    <>
      <Datamate
        clientId={process.env.REACT_APP_DATAMATE_CLIENT_ID!}
        trackHashChanges
        trackAttributes
        trackOutgoingLinks
		trackWebVitals
...
Use `trackError` for manual error tracking. For automatic JavaScript error capture, enable `trackErrors` on the `<Datamate />` component.
...
function App() {
  ...
  return (
    <>
      {shouldTrack && (
        <Datamate clientId={process.env.REACT_APP_DATAMATE_CLIENT_ID!} />
      )}
      <YourAppContent />
    </>
  );
...
```tsx
import { useCallback } from 'react';
import { track, trackError } from '@datamate/sdk/react';
...
Need help with React integration? Contact us at [support@datamate.cc](mailto:support@datamate.cc).
