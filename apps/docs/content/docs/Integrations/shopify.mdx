...
Add Datamate's privacy-first analytics to your Shopify store to track customer behavior, e-commerce events, and improve conversion rates while maintaining GDPR compliance.

## Installation Methods

### Method 1: Theme Code (Recommended)

Add Datamate directly to your Shopify theme:

1. Go to **Online Store** > **Themes**
2. Click **Actions** > **Edit code** on your active theme
3. Open `theme.liquid` in the Layout folder
4. Add your Datamate script before the closing `</head>` tag:
...
<script>
  window.addEventListener('load', function() {
    if (window.datamate) {
      datamate.track('collection_view', {
...
<script>
  ...
  document.addEventListener('cart:updated', function(event) {
    if (window.datamate && event.detail.cart.item_count > 0) {
      datamate.track('cart_updated', {
...
Load Datamate after critical resources:

```html
<script>
  window.addEventListener('load', function() {
    // Load Datamate after page is fully loaded
    var script = document.createElement('script');
    script.src = 'https://cdn.datamate.cc/datamate.js';
    script.setAttribute('data-client-id', 'YOUR_CLIENT_ID');
    script.async = true;
...
Datamate is GDPR compliant by default, but you can add additional privacy controls:

```html
<!-- Show only for EU users -->
{% if localization.country.iso_code == 'EU' %}
<script>
  // Initialize Datamate with privacy mode
...
document.addEventListener('cookieConsentAccepted', initializeDatamate);
```
...
3. Verify events in your Datamate dashboard
...
Need help with your Shopify integration? Contact us at [support@datamate.cc](mailto:support@datamate.cc).
