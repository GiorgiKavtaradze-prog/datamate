...
This guide explains how to integrate Datamate with your SvelteKit application for analytics tracking.

## How to Add Datamate to Your SvelteKit App

The recommended method for SvelteKit is to add the Datamate tracking script directly to your `src/app.html` file. This ensures the script is loaded early on all pages.

### Adding to `src/app.html`

<Steps>
  <Step title="Get Your Tracking Script">
    Navigate to your [Datamate dashboard](https://app.datamate.cc) to get your tracking code snippet:
...
></script>`}
  ...
  <Step title="Add the Snippet to app.html">
    Open `src/app.html` and paste the Datamate tracking snippet into the `<head>` section, usually before `%sveltekit.head%` or just after it.
...
<html lang="en">
  ...
  <Step title="Verify Installation">
    Deploy your application and check your [Datamate dashboard](https://app.datamate.cc) for incoming data. You can also inspect your browser's network tab to confirm the script is loaded.
  </Step>
</Steps>

<Callout type="info">
  **SPA Page View Tracking**: Datamate automatically tracks route changes in SvelteKit applications. If page views aren't tracked correctly after navigation (which is uncommon), you can manually trigger them using the `afterNavigate` lifecycle function:
...
Always check `typeof window !== 'undefined'` before accessing `window.datamate` as SvelteKit components can also run on the server during SSR.
...
<CodeBlock
  ...
  function handleResult({ result }) {
    if (result.type === 'success' && typeof window !== 'undefined' && window.datamate) {
      window.datamate.track('form_submit_success', {
        form_type: 'contact'
      });
    }
  }
...
<CodeBlock
  ...
  onMount(() => {
    if (typeof window !== 'undefined' && window.datamate) {
      window.datamate.track('pricing_page_view', {
  ...
  function trackPlanClick(planName) {
    if (typeof window !== 'undefined' && window.datamate) {
      window.datamate.track('plan_clicked', {
        plan_name: planName,
        page_path: window.location.pathname
      });
    }
...
For server-side tracking in SvelteKit, use the Datamate Node SDK:
...
<CodeBlock
  language="typescript"
  filename="src/lib/datamate.ts"
  code={`import { Datamate } from '@datamate/sdk/node';
import { DATAMATE_API_KEY, DATAMATE_WEBSITE_ID } from '$env/static/private';

export const datamate = new Datamate({
  apiKey: DATAMATE_API_KEY,
  websiteId: DATAMATE_WEBSITE_ID
});
...
<CodeBlock
  language="bash"
  filename=".env"
  code={`PUBLIC_DATAMATE_CLIENT_ID=your-client-id-here
...
<CodeBlock
  ...
  code={`<script
  src="https://cdn.datamate.cc/datamate.js"
  data-client-id="%env.PUBLIC_DATAMATE_CLIENT_ID%"
  async
></script>`}
/>
...
- Datamate automatically tracks route changes in SvelteKit
...
- Never call `window.datamate` during SSR (server-side rendering)
...
Need help with your SvelteKit integration? Contact us at [support@datamate.cc](mailto:support@datamate.cc).
