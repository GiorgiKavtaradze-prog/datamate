...
`@datamate/nuxt` is a first-class Nuxt module. Add it to `nuxt.config.ts` once and you're done — pageviews and SPA navigation work automatically with no `app.vue` changes. Enable `trackErrors` to also capture JavaScript and Vue component errors.
...
<CodeBlock language="bash">
  {`npx nuxi module add @datamate/nuxt`}
</CodeBlock>

Or install manually:

<CodeBlock language="bash">
  {`bun add @datamate/nuxt
# npm install @datamate/nuxt
# yarn add @datamate/nuxt
# pnpm add @datamate/nuxt`}
</CodeBlock>
...
<CodeBlock language="ts" filename="nuxt.config.ts">
  ...
  modules: ["@datamate/nuxt"],
  datamate: {
    clientId: process.env.NUXT_PUBLIC_DATAMATE_CLIENT_ID,
  },
})`}
</CodeBlock>

<CodeBlock language="bash" filename=".env">
  {`NUXT_PUBLIC_DATAMATE_CLIENT_ID=your-client-id`}
</CodeBlock>
...
<Callout type="info">
  Nuxt automatically maps the `NUXT_PUBLIC_DATAMATE_CLIENT_ID` environment variable to `runtimeConfig.public.datamate.clientId` — no extra configuration needed.
</Callout>

## Tracking Events

`useDatamate` is auto-imported in every component:
...
All functions are available from `useDatamate()`:
...
`$datamate` is available directly in templates and Options API components — no import needed:

<CodeBlock language="html">
  {`<template>
  <button @click="$datamate.track('cta_clicked')">Sign up</button>
</template>`}
</CodeBlock>
...
<CodeBlock language="ts">
  ...
  methods: {
    handleClick() {
      this.$datamate.track("cta_clicked")
    },
  },
})`}
</CodeBlock>
...
All options are passed under the `datamate` key in `nuxt.config.ts`:

| Option | Type | Default | Description |
|--------|------|---------|-------------|
| `clientId` | `string` | — | Your Datamate client ID |
