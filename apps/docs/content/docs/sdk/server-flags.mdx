...
The Datamate Node SDK includes a server-side feature flags manager optimized for server environments with request deduplication, batching, and stale-while-revalidate caching.

<Callout type="info">
  **Package**: `@datamate/sdk` | **Import**: `@datamate/sdk/node`
</Callout>
...
<CodeBlock language="tsx" filename="app/api/export/route.ts">
  {`import { Datamate, createServerFlagsManager } from "@datamate/sdk/node";

const events = new Datamate({
