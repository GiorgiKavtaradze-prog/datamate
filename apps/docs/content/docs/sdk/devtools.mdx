---
title: Datamate DevTools
description: Inspect Datamate events, identity, queues, diagnostics, and feature flags during local development
---

import { Callout, CodeBlock, Card, Cards } from "@/components/docs";

Datamate DevTools is a moveable browser overlay for local development, QA, and previews. It observes the Datamate browser SDK already running on the page.

<Callout type="info">
  **Package**: `@datamate/devtools` | **Shortcut**: Cmd/Ctrl + Shift + D
</Callout>
...
The default API host is the active flag manager API URL, falling back to `https://api.datamate.cc`. You can override it in the overlay for local or preview environments.
