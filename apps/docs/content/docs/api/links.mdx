...
- **Filter invites** — Companies can tag invite links with their ID and list all links for that company
- **Attribute traffic** — Query which external entity drove clicks
- **Integrate** — Sync with your CRM, partner dashboard, or internal systems

Set `externalId` when creating or updating a link. Use `externalId` in the list endpoint to filter:

<CodeBlock 
  language="json"
  code={`// Create link with external ID
{
