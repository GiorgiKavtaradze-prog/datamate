...
| `websiteId` | string | No | Public Datamate Client ID (same value as `data-client-id` in the tracker; string id from the dashboard, not necessarily a UUID). Alternative to `?website_id=` |
...
<CodeBlock 
  ...
  const response = await fetch('https://basket.datamate.cc/track', {
    method: 'POST',
    headers: {
...
<CodeBlock 
  language="bash"
  code={`curl -X POST https://basket.datamate.cc/track \\
...
These endpoints are used by the JavaScript tracker SDK and are documented here for completeness. The `client_id` query value is your Datamate Client ID.
