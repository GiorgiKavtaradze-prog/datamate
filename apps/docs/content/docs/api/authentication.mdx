...
description: API keys, scopes, and authentication methods for the Datamate API
---

import { Callout, CodeBlock } from "@/components/docs";

Protected API operations require authentication. Operations explicitly documented as public—such as query-type discovery and public status-page reads—do not. Datamate supports API keys for server-side integrations and session cookies for browser-based apps.
...
AI agents can discover Datamate authentication without scraping this page:

| Resource | URL |
|----------|-----|
| auth.md walkthrough | `https://www.datamate.cc/auth.md` |
| OAuth Protected Resource Metadata | `https://api.datamate.cc/.well-known/oauth-protected-resource` |
| Authorization Server Metadata | `https://api.datamate.cc/.well-known/oauth-authorization-server` |
| API catalog | `https://api.datamate.cc/.well-known/api-catalog` |

Unauthenticated protected API probes return a `WWW-Authenticate` header with `Bearer resource_metadata="https://api.datamate.cc/.well-known/oauth-protected-resource"`. Agents should fetch that metadata, choose the smallest required scope, and then retry with `x-api-key` or `Authorization: Bearer`.
...
Browser-based applications using the Datamate dashboard session can authenticate automatically via cookies. This works when:

- Users are logged into the Datamate dashboard
- Requests include `credentials: 'include'`
- Requests originate from `*.datamate.cc` domains

<CodeBlock 
  language="typescript"
  code={`fetch('https://api.datamate.cc/v1/query/websites', {
  credentials: 'include'
})`}
/>
