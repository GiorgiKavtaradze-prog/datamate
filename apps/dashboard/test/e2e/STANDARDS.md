...
E2E tests prove that critical Datamate user journeys work in a real browser. They are not a replacement for unit, integration, or query-level tests.
...
```bash
DATAMATE_E2E_KEEP_DB=true bun run --cwd apps/dashboard test:e2e:local
DATAMATE_E2E_CLICKHOUSE_EVENTS=1000 bun run --cwd apps/dashboard test:e2e:local
DATAMATE_E2E_SEED_CLICKHOUSE=false bun run --cwd apps/dashboard test:e2e:local
```
