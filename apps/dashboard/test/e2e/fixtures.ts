...
function e2eTestKey(): string {
	...
	if (!key) {
		throw new Error(
			"DATAMATE_E2E_TEST_KEY is required. Run through test:e2e:local."
		);
	}
	return key;
}
...
async function seedClickHouse(
	...
	const response = await request.post("/api/test/e2e/clickhouse", {
		data: {
			eventCount: process.env.DATAMATE_E2E_CLICKHOUSE_EVENTS ?? "250",
			websiteId,
		},
	...
	e2eSession: async ({ page }, use, testInfo) => {
		const response = await page
			...
			.request.post("/api/test/e2e/session", {
				data: {
					runScope: process.env.DATAMATE_E2E_RUN_ID ?? "local",
