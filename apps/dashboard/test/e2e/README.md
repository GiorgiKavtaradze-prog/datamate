...
Set `DATAMATE_E2E_KEEP_DB=true` to keep the Postgres database for debugging.
...
```bash
DATAMATE_E2E_START_CLICKHOUSE=false  # do not start docker compose clickhouse
DATAMATE_E2E_SEED_CLICKHOUSE=false  # do not seed per-test analytics data
DATAMATE_E2E_CLICKHOUSE_EVENTS=500   # seed size per test website
CLICKHOUSE_URL=http://default:@localhost:8123/datamate_analytics
```

## Session bootstrap

When `DATAMATE_E2E_MODE=true` and `DATAMATE_E2E_TEST_KEY` is set, tests can create a signed-in user via:

```http
POST /api/test/e2e/session
x-e2e-test-key: <DATAMATE_E2E_TEST_KEY>
