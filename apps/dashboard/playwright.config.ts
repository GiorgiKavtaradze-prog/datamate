...
const PORT = Number(process.env.DATAMATE_E2E_PORT ?? 3000);
...
export default defineConfig({
	...
	webServer: {
		...
		env: {
			DATAMATE_E2E_MODE: process.env.DATAMATE_E2E_MODE ?? "true",
			NEXT_PUBLIC_DATAMATE_E2E_MODE:
				process.env.NEXT_PUBLIC_DATAMATE_E2E_MODE ?? "true",
			DATAMATE_E2E_TEST_KEY: process.env.DATAMATE_E2E_TEST_KEY ?? "",
			...
			BETTER_AUTH_SECRET:
				...
				"datamate-e2e-secret-at-least-32-bytes",
		...
		reuseExistingServer: readBooleanEnv("DATAMATE_E2E_REUSE_SERVER"),
		timeout: 120_000,
		url: baseURL,
	},
	projects: [
