...
import { Switch, Tabs } from "@datamate/ui/client";
...
const INSTALL_COMMANDS = {
	bun: "bun add @datamate/sdk",
	npm: "npm install @datamate/sdk",
	yarn: "yarn add @datamate/sdk",
	pnpm: "pnpm add @datamate/sdk",
};
...
const TROUBLESHOOTING_ITEMS = [
	...
	},
	{
		...
		description:
			"Check Developer Tools (F12) Network tab for the datamate.js request. Verify the script is in <head> and your Client ID is correct.",
	},
	{
		title: "Ad blockers",
		description:
...
export function WebsiteTrackingSetupTab({ websiteId }: TrackingSetupTabProps) {
	...
	const activeVersionedScript = useMemo((): VersionedScript | undefined => {
		...
		return {
			...
			filename: `datamate.v${match.version}.js`,
					...
					<Tabs className="w-full" defaultValue="script">
						...
						<Tabs.Panel className="mt-4 space-y-4" value="node">
							...
							<div className="rounded-lg border border-border/60 bg-accent/40 p-3 text-muted-foreground text-sm">
								Use a Datamate API key for server-side events. Create one in{" "}
								<Link
							...
							<a
								...
								href="https://www.datamate.cc/docs/sdk/node"
								rel="noreferrer"
								target="_blank"
							>
							...
							<div className="mt-4 flex items-center gap-2 rounded-lg bg-accent/50 p-3">
								...
								<p className="text-muted-foreground text-xs">
									...
									<a
										...
										href="https://www.datamate.cc/docs/troubleshooting"
