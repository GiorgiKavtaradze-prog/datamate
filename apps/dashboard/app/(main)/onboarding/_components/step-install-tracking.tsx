...
import { Badge, Button, Card } from "@datamate/ui";
import { Tabs } from "@datamate/ui/client";
import {
	ArrowClockwiseIcon,
	CheckIcon,
	ClipboardIcon,
...
const SKILL_URL = "https://github.com/datamate-cc/skill";
...
function generateAgentPrompt(websiteId: string): string {
	return `Add Datamate analytics to this repository. Client ID: ${websiteId}

## References
- Docs: https://www.datamate.cc/docs/getting-started
- LLMs.txt: https://www.datamate.cc/llms.txt
- Full docs: https://www.datamate.cc/docs
...
**React / Next.js** — \`bun add @datamate/sdk\` (or npm/yarn/pnpm)
\`\`\`tsx
import { Datamate } from "@datamate/sdk/react";
// Mount at the app root (layout.tsx or _app.tsx)
<Datamate clientId={process.env.NEXT_PUBLIC_DATAMATE_CLIENT_ID!} />
\`\`\`

**Vue** — \`bun add @datamate/sdk\`
\`\`\`vue
<script setup>
import { Datamate } from "@datamate/sdk/vue";
</script>
<template>
  <Datamate :client-id="import.meta.env.VITE_DATAMATE_CLIENT_ID" />
</template>
\`\`\`

**Vanilla JS / HTML** — CDN script in \`<head>\`:
\`\`\`html
<script src="https://cdn.datamate.cc/datamate.js" data-client-id="${websiteId}" crossorigin="anonymous" async></script>
\`\`\`

Store the Client ID in an env var — never hardcode it.
- Next.js: NEXT_PUBLIC_DATAMATE_CLIENT_ID
- Vue/Vite: VITE_DATAMATE_CLIENT_ID
...
\`\`\`tsx
<Datamate clientId={...} trackWebVitals trackErrors />
\`\`\`
...
2. Look for a request to cdn.datamate.cc/datamate.js (script loading)
3. Look for requests to basket.datamate.cc (events being sent)
4. Both should return 200. If events show the correct Client ID in the payload, tracking is working.

## Common Issues & Fixes

**Domain mismatch**: Events are rejected if sent from a domain that doesn't match the website configured in Datamate. The domain in settings must match the domain the script runs on.

**Content Security Policy (CSP)**: If the site has strict CSP headers, add these directives:
- script-src: https://cdn.datamate.cc
- connect-src: https://basket.datamate.cc
...
**Events not appearing in dashboard**: Data typically appears within 30 seconds. Check the Network tab for failed requests to basket.datamate.cc. Verify the Client ID matches. Check for console errors.

**If another analytics tool is present**: Both can run in parallel. No conflicts. Optionally disable the other tool's page view tracking if Datamate handles it.`;
}
		...
		<div className="space-y-6">
			...
			<Tabs className="w-full" defaultValue="ai">
				...
				<Tabs.Panel className="mt-4 space-y-4" value="ai">
					<p className="text-pretty text-muted-foreground text-sm">
						Let your AI assistant install Datamate for you. Copy the setup
