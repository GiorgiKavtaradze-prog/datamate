...
import { Accordion } from "@datamate/ui/client";
import {
	ArrowRightIcon,
	ArrowSquareOutIcon,
	CheckCircleIcon,
...
type DatamateSetupRecommendation = Extract<
	Recommendation,
	{ kind: "datamate_setup" }
>;
		...
		return (
			<div className="px-5 py-12">
				<EmptyState
					...
					description="Datamate couldn't load current recommendations."
...
function recommendationAction(insight: InsightRecommendation) {
	...
	if (isInstrumentationRecommendation(recommendation)) {
		return (
			<Button asChild size="sm">
				<Link
					href="https://www.datamate.cc/docs/hooks"
					rel="noreferrer"
					target="_blank"
				>
	...
	if (isDatamateSetupRecommendation(recommendation)) {
		if (recommendation.feature === "user_identification") {
			return (
				<Button asChild size="sm">
					<Link
						href="https://www.datamate.cc/docs/sdk/identify-users"
						rel="noreferrer"
						target="_blank"
					>
...
function isDatamateSetupRecommendation(
	recommendation: Recommendation
): recommendation is DatamateSetupRecommendation {
	return "kind" in recommendation && recommendation.kind === "datamate_setup";
}
