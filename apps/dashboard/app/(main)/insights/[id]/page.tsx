				...
				<header className="flex min-w-0 items-center gap-2 text-xs">
					...
					<div className="flex min-w-0 items-center gap-2">
						<span className="truncate font-medium text-foreground">
							{item.kind === "reply" ? item.author : "Datamate"}
						</span>
...
function ReplyComposer({
	...
	const submitReply = (event: FormEvent<HTMLFormElement>) => {
		...
		sendReply(trimmed, "Datamate is checking the latest context");
	};
...
function scheduledRecheck(
	...
	return `Datamate will check again ${dayjs.utc(next.recheckAt).format("MMM D")}`;
}
