import { clickHouse, TABLE_NAMES } from "@datamate/db/clickhouse";
...
function isE2EModeEnabled(): boolean {
	return readBooleanEnv("DATAMATE_E2E_MODE");
}
...
function assertE2EAccess(request: Request): Response | null {
	...
	const key = process.env.DATAMATE_E2E_TEST_KEY;
	if (!key) {
		return notFound();
	}
...
export async function POST(request: Request): Promise<Response> {
	...
	if (!readBooleanEnv("DATAMATE_E2E_SEED_CLICKHOUSE")) {
	...
	const events = Array.from({ length: eventCount }, (_, index) => {
		...
		return {
			...
			url: `https://e2e.datamate.local${path}`,
			path,
			title: pageTitle(path),
			ip: "127.0.0.1",
			user_agent: "Datamate E2E",
	...
	const outgoingLinks = sessions
		...
		.map((session) => ({
			...
			anonymous_id: session.anonymousId,
			session_id: session.sessionId,
			href: "https://github.com/datamate-analytics/Datamate",
			text: "Datamate GitHub",
			properties: "{}",
