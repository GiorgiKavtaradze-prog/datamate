...
describe("deep-link fallback page", () => {
	test("tries the native app and keeps an HTTPS browser fallback", () => {
		const page = renderDeepLinkFallbackPage(
			"instagram://user?username=datamate",
			"https://www.instagram.com/datamate"
		);

		expect(page).toContain('const appUrl = "instagram://user?username=datamate"');
		expect(page).toContain(
			'const fallbackUrl = "https://www.instagram.com/datamate"'
		);
		expect(page).toContain('href="https://www.instagram.com/datamate"');
