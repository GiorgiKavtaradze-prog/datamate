...
mock.module("@datamate/shared/bot-detection", () => ({
	BotCategory: {
...
mock.module("@datamate/shared/constants/deep-link-apps", () => ({
	resolveDeepLink,
}));
	...
	test("serves a mobile deep-link launcher with a safe HTTPS fallback", async () => {
		getCachedLink.mockResolvedValue({
			link: {
				...
				targetUrl: "https://www.instagram.com/datamate",
			},
			state: "hit",
		});
		resolveDeepLink.mockReturnValue("instagram://user?username=datamate");

		const response = await app.handle(
			new Request("http://links.test/datamate", {
				headers: {
					"user-agent":
		...
		expect(body).toContain("instagram://user?username=datamate");
		expect(body).toContain("https://www.instagram.com/datamate");
