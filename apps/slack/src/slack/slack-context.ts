...
export function createSlackConversationContext(
	...
	run: SlackAgentRun
): DatamateAgentSlackContext | null {
...
function mapSlackApiMessages(result: unknown): DatamateAgentSlackMessage[] {
	if (!(isRecord(result) && Array.isArray(result.messages))) {
		return [];
	}

	return result.messages
		.map((message): DatamateAgentSlackMessage | null => {
			if (!isRecord(message)) {
				return null;
			}

			const text = getString(message.text)?.trim();
			const mapped: DatamateAgentSlackMessage = {
		...
		.filter(
			(message): message is DatamateAgentSlackMessage => message !== null
		);
}
