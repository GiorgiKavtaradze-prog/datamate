...
import { and, db, desc, eq, or, sql } from "@datamate/db";
import {
	insightObservations,
	insightRunEffects,
	insightRunItems,
...
import { createRPCContext } from "@datamate/rpc";
import { appendInvestigationReply } from "@datamate/rpc/insights";
import type { DatamateAgentClient, SlackAgentRun } from "@/agent/agent-client";
...
function createDatamateAssistant({
	agent,
	dedupe,
	threadQueue,
}: {
	agent: Pick<DatamateAgentClient, "stream">;
...
export function registerSlackListeners(
	...
	app.assistant(createDatamateAssistant({ agent, dedupe, threadQueue }));
...
function registerSlackCommands(
	...
	app.command("/datamate-help", async ({ ack, respond }) => {
		await ack();
		await respond({ response_type: "ephemeral", text: SLACK_COPY.help });
	});

	app.command("/datamate-status", async ({ ack, command, logger, respond }) => {
		await ack();
		await respondToStatusCommand({ command, installations, logger, respond });
	});

	app.command("/datamate-bind", async ({ ack, command, logger, respond }) => {
		await ack();
