...
describe("Slack thread reply relevance", () => {
	...
	it("lets the model allow the exact short clarification answer from thread context", async () => {
		...
		await expect(
			decideWithThread("both", [
				...
				{
					text: "I see two websites — Datamate (app.datamate.cc) and Landing Page (datamate.cc). Which one's top pages would you like me to pull?",
					userId: "UBOT",
				},
			])
	...
	it("lets the model answer relay requests to another human after Datamate spoke", async () => {
		modelDecision = {
			confidence: 0.9,
			reason: "direct_request",
			shouldReply: true,
	...
	it("falls back conservatively for unmentioned replies when the model is unavailable", async () => {
		await expect(decide("datamate is gonna make qais mad")).resolves.toMatchObject({
			reason: "ambiguous",
			shouldReply: false,
			source: "fallback",
		});
