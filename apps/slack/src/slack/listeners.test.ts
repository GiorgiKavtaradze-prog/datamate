...
function createAgent() {
	...
	const agent: Pick<DatamateAgentClient, "stream"> = {
...
describe("Slack listeners", () => {
	...
	it("ignores channel thread replies that Datamate has not joined", async () => {
		const app = new FakeSlackApp();
		const { agent, runs } = createAgent();
	...
	it("lets app_mention handle threaded messages that mention Datamate", async () => {
		const app = new FakeSlackApp();
		const { agent, runs } = createAgent();
		const queue = createQueue();
		const { client } = createClient();
	...
	it("responds to the help slash command", async () => {
		...
		await app.commands.get("/datamate-help")?.({
			ack: async () => undefined,
