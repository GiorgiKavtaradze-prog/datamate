...
const DASHBOARD_URL = "https://app.datamate.cc";
...
export function buildAppHomeView(sites: ConnectedSite[] = []): HomeView {
	...
	return {
		...
		blocks: [
			{
				type: "header",
				text: { type: "plain_text", text: "Datamate" },
			},
			{
				type: "section",
				text: {
					type: "mrkdwn",
					text: "Ask about your analytics right here in Slack — traffic, pages, conversions, campaigns, errors, and product usage. Mention *@Datamate*, send a direct message, or use the assistant.",
				},
			},
			...
			{ type: "divider" },
			{
				...
				elements: [
					{
						type: "mrkdwn",
						text: "Commands: `/datamate-status`   `/datamate-help`   `/datamate-bind`",
					},
				],
			},
		],
