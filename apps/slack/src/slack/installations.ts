...
import { slackChannelBindings, slackIntegrations } from "@datamate/db/schema";
import { decrypt } from "@datamate/encryption";
import {
	cacheNamespaces,
	cacheable,
	invalidateSlackChannelBindingCache,
} from "@datamate/redis";
import { createInternalPrincipal } from "@datamate/rpc";
	...
	async authorize(teamId: string) {
		...
		try {
			...
			if (!installation) {
				...
				throw new Error(`Slack team ${teamId} is not connected to Datamate`);
			}
