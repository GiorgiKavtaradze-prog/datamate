...
import { isDatamateAgentUserError } from "@datamate/ai/agent/errors";
...
export async function streamAgentToSlack({
	...
	try {
		...
		const failureText = isDatamateAgentUserError(error)
			? error.message
...
function logStreamError(
	...
	const userFacingError = isDatamateAgentUserError(error) ? error : null;
