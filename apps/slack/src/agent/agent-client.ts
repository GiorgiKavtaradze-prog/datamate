import type { DatamateAgentSlackContext } from "@datamate/ai/agent";
...
class SharedDatamateAgentRunner implements SlackAgentRunner {
	async *stream(
		run: SlackAgentRun,
		context: SlackRunContext,
		options?: SlackAgentStreamOptions
		...
		const { streamDatamateAgent } = await import("@datamate/ai/agent");

		yield* streamDatamateAgent({
