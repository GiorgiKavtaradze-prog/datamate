...
import { and, db, eq } from "@datamate/db";
import { member, slackIntegrations } from "@datamate/db/schema";
import { encrypt } from "@datamate/encryption";
import { config } from "@datamate/env/app";
import { invalidateSlackIntegrationCache } from "@datamate/redis/cache-invalidation";
...
function requireConfig(_request: Request): SlackOAuthConfig {
	...
	const encryptionKey = process.env.DATAMATE_ENCRYPTION_KEY;
	...
	if (!encryptionKey) {
		missing.push("DATAMATE_ENCRYPTION_KEY");
	}
	if (
		missing.length > 0 ||
