import {
	...
	parseNlwebAskBody,
} from "@datamate/shared/agent-discovery";
import { config } from "@datamate/env/app";
import { Elysia } from "elysia";

const SITE_URL = "https://www.datamate.cc";
	...
	return noStoreJsonResponse(
		{
			...
			code: "AUTH_REQUIRED",
			fix: `Fetch ${discoveryUrls.protectedResourceMetadataUrl}, then send a scoped Datamate API key in x-api-key or Authorization: Bearer.`,
		},
		{
			status: 401,
			headers: {
...
export const discovery = new Elysia({ name: "agent-discovery" })
	...
	.post("/checkout_sessions", () =>
		acpErrorResponse(
			...
			"Datamate agent checkout sessions require an authenticated billing context."
		)
	)
	...
	.post("/agentic_commerce/delegate_payment", () =>
		acpErrorResponse(
			...
			"Datamate delegate payments require an authenticated billing context."
		)
	)
