...
vi.mock("@datamate/email", () => ({
  render: vi.fn(async () => "<html />"),
  UsageAlertEmail: vi.fn(() => ({ type: "usage" })),
  UsageLimitEmail: vi.fn(() => ({ type: "limit" })),
}));

vi.mock("@datamate/env/app", () => ({
  config: { email: { alertsFrom: "alerts@datamate.cc" } },
}));

vi.mock("@datamate/notifications", () => ({
  SlackProvider: class {
    send = vi.fn(async () => undefined);
  },
}));

vi.mock("@datamate/redis", () => ({
...
vi.mock("@datamate/rpc", () => ({
  getAutumn: () => ({ check: state.check }),
}));

vi.mock("@datamate/services/billing-lifecycle", () => ({
...
import { UsageAlertEmail, UsageLimitEmail } from "@datamate/email";
import {
