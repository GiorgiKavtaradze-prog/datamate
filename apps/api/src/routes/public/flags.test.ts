...
describe("evaluateStringRule - email pattern matching", () => {
	...
	it("handles ends_with for email domain patterns", () => {
		...
		expect(
			evaluateStringRule(
				"user@datamate.cc",
				emailRule("ends_with", "@datamate.cc")
			)
		).toBe(true);
		expect(
			evaluateStringRule(
	...
	it("handles exact match for full email addresses", () => {
		expect(
			evaluateStringRule(
				"user@datamate.cc",
				emailRule("equals", "user@datamate.cc")
			)
		).toBe(true);
		expect(
			evaluateStringRule(
...
describe("evaluateRule", () => {
	...
	it("handles batch rules with ends_with for email domain targeting", () => {
		const emailEndsWithRule = {
			...
			batch: true,
			batchValues: ["@datamate.cc"],
			enabled: true,
		};
		expect(
			evaluateRule(emailEndsWithRule, { email: "user@datamate.cc" })
		).toBe(true);
		expect(
			evaluateRule(emailEndsWithRule, { email: "admin@datamate.cc" })
		).toBe(true);
