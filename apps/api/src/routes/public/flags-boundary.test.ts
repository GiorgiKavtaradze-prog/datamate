...
vi.mock("@datamate/db", async (importOriginal) => ({
	...(await importOriginal<typeof import("@datamate/db")>()),
	db: {
		query: {
			flags: {
...
vi.mock("@datamate/redis", async (importOriginal) => ({
	...(await importOriginal<typeof import("@datamate/redis")>()),
	cacheable: (fn: (...args: never[]) => unknown) => fn,
}));

vi.mock("@datamate/redis/rate-limit", () => ({
	getRateLimitHeaders: () => ({}),
