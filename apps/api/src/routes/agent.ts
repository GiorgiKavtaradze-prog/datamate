import {
	...
	hasKeyScope,
	isApiKeyPresent,
} from "@datamate/api-keys/resolve";
import { createConfig as createAgentConfig } from "@datamate/ai/agents/analytics";
import {
	ensureAgentCreditsAvailable,
	resolveAgentBillingCustomerId,
	trackAgentUsageAndBill,
} from "@datamate/ai/agents/execution";
import { type AgentTier, tierToModelKey } from "@datamate/ai/agents/router";
import {
	AGENT_THINKING_LEVELS,
	AGENT_TIERS,
	type AgentConfig,
} from "@datamate/ai/agents/types";
import {
	type AgentModelKey,
...
import { askDatamateAgent, streamDatamateAgent } from "@datamate/ai/agent";
import {
	formatMemoryForPrompt,
	isMemoryEnabled,
	storeConversation,
	type MemoryContext,
} from "@datamate/ai/lib/supermemory";
import { auth } from "@datamate/auth";
import { db, eq } from "@datamate/db";
import { agentChats } from "@datamate/db/schema";
...
import {
	...
	streamBufferKey,
	tailStream,
} from "@datamate/redis/stream-buffer";
import { ratelimit } from "@datamate/redis/rate-limit";
import {
...
} from "@datamate/ai/agents/cache";
import { getAILogger } from "@datamate/ai/lib/ai-logger";
import { trackAgentEvent } from "@datamate/ai/lib/datamate";
import { getResolvedAuth } from "../lib/auth-wide-event";
import { captureError, mergeWideEvent } from "@datamate/ai/lib/tracing";
import { getAccessibleWebsites } from "@datamate/ai/lib/accessible-websites";
	...
	.post(
		...
		async function agentAsk({ body, user, apiKey, request }) {
			...
			try {
				...
				const result = await askDatamateAgent({
					actor,
					conversationId,
					input: body.question,
					source: "slack",
			...
			return (async () => {
				...
				try {
					...
					const agent = createToolLoopAgent(config, {
						isEnabled: true,
						functionId: `datamate.dashboard.agent.${AGENT_TYPE}`,
