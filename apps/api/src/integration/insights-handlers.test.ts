...
import { createMcpTools } from "@datamate/ai/mcp/tools";
import { eq } from "@datamate/db";
import {
	analyticsInsights,
	funnelDefinitions,
	goals,
...
import type { InvestigationOutcome } from "@datamate/shared/insights";
import {
	addToOrganization,
	cleanup,
	db,
...
describe("insight investigation timeline", () => {
	...
	iit("hides a case from the action inbox while a reply is being verified", async () => {
		...
		await db().insert(insightReplies).values({
			...
			body: "Datamate applied the suggested action.",
			id: randomUUIDv7(),
			insightId,
			status: "running",
		});
	...
	iit("applies an executable goal action and queues verification together", async () => {
		...
		expect(applied.reply).toMatchObject({
			body: "Datamate applied the goal action. Recheck its verification condition against current data.",
			kind: "reply",
			status: "queued",
		});
		expect(
	...
	iit("applies an executable funnel action and queues verification together", async () => {
		...
		expect(applied.reply).toMatchObject({
			body: "Datamate applied the funnel action. Recheck its verification condition against current data.",
			kind: "reply",
			status: "queued",
		});
		expect(
	...
	iit("remeasures an open goal investigation after a teammate edits its definition", async () => {
		...
		expect(replies).toEqual([
			expect.objectContaining({
				authorName: "Datamate",
				body: "Datamate detected a goal definition change. Recheck the current evidence and resolve this investigation if the change addressed it.",
				status: "queued",
			}),
		]);
		expect(
	...
	iit("remeasures an open funnel-step investigation after its funnel changes", async () => {
		...
		expect(
			...
			{
				body: "Datamate detected a funnel definition change. Recheck the current evidence and resolve this investigation if the change addressed it.",
				status: "queued",
			},
		]);
	});
	...
	iit("expires standalone setup recommendations at their renewal deadline", async () => {
		...
		const standaloneRecommendation = (
			action: string,
			kind: "datamate_setup" | "instrumentation"
		): InvestigationOutcome => ({
