...
vi.mock("@datamate/redis/rate-limit", () => ({ ratelimit: vi.fn() }));
...
import { VALIDATION_LIMITS as SHARED_LIMITS } from "@datamate/validation";
...
describe("pii protection", () => {
  ...
  afterEach(() => {
    delete process.env.DATAMATE_ENCRYPTION_KEY;
  });

  test("encrypts and reveals round-trip when a key is configured", () => {
    process.env.DATAMATE_ENCRYPTION_KEY = KEY;
  ...
  test("returns null for undecryptable payloads instead of leaking them", () => {
    process.env.DATAMATE_ENCRYPTION_KEY = KEY;
    const protectedValue = protectPii("jo@acme.com");
    process.env.DATAMATE_ENCRYPTION_KEY = "different-key";
