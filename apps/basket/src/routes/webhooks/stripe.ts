...
function buildAnalyticsMetadata(
	...
	const anonymousId =
		metadata.datamate_anonymous_id && dailySalt
			? saltAnonymousId(metadata.datamate_anonymous_id, dailySalt)
			: undefined;
	const clientId = sanitizeString(
		metadata.datamate_client_id,
		VALIDATION_LIMITS.USER_ID_MAX_LENGTH
	);
	const profileId = sanitizeString(
		metadata.datamate_profile_id,
		VALIDATION_LIMITS.USER_ID_MAX_LENGTH
	);
	const sessionId = sanitizeString(
		metadata.datamate_session_id,
...
async function insertStripeRevenue(
	...
	const needsAnonymousSalt = records.some(
		(record) => record.rawMetadata.datamate_anonymous_id
	);
