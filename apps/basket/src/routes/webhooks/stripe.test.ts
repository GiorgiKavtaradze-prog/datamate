...
describe("getInvoiceMetadata", () => {
	test("merges parent, subscription_details, and invoice metadata with invoice winning", () => {
		const merged = getInvoiceMetadata({
			...
			currency: "usd",
			id: "in_1",
			metadata: { datamate_profile_id: "user_invoice" },
			parent: {
				subscription_details: {
					metadata: {
						datamate_profile_id: "user_parent",
						datamate_client_id: "site_parent",
					},
				},
			},
			subscription_details: {
				metadata: { datamate_session_id: "sess_sub" },
			},
		});
		expect(merged).toEqual({
			datamate_profile_id: "user_invoice",
			datamate_client_id: "site_parent",
			datamate_session_id: "sess_sub",
		});
	});
...
describe("normalizeStripeEvent", () => {
	const modernIntent = {
		...
		data: {
			object: {
				...
				id: "pi_1",
				metadata: {
					datamate_profile_id: "profile-1",
					datamate_session_id: "session-1",
				},
			},
		},
	...
	test("carries expanded invoice context on direct InvoicePayment events", () => {
			...
			data: {
				object: {
					...
					invoice: {
						...
						id: "in_expanded",
						metadata: { datamate_session_id: "session-invoice" },
						parent: {
							subscription_details: {
								metadata: { datamate_profile_id: "profile-subscription" },
							},
						},
					},
					payment: {
						payment_intent: {
							...
							id: "pi_expanded",
							metadata: { datamate_anonymous_id: "anon-payment" },
						},
						type: "payment_intent",
					},
					status: "paid",
		...
		expect(record).toMatchObject({
			...
			rawMetadata: {
				datamate_anonymous_id: "anon-payment",
				datamate_profile_id: "profile-subscription",
				datamate_session_id: "session-invoice",
			},
			context: {
